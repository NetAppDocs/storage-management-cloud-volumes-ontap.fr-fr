...
La méthode préférée pour exporter une image vers Cloud Storage est d'utiliser le https://cloud.google.com/sdk/gcloud/reference/compute/images/export["commande d'exportation d'images de calcul gcloud"^] .  Cette commande prend l'image fournie et la convertit en un fichier disk.raw qui est compressé et compressé.  Le fichier généré est enregistré à l'URL de destination et peut ensuite être téléchargé pour vérification.
...
NOTE: Pour plus d'informations sur la façon d'exporter une image via Google Cloud, reportez-vous à la https://cloud.google.com/compute/docs/images/export-image#console["Documentation Google Cloud sur l'exportation d'une image"^] .
