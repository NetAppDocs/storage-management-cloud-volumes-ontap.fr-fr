...
.Étapes
. Sélectionnez la vue System Manager pour redimensionner un volume via ONTAP System Manager. Reportez-vous à link:task-administer-advanced-view.html#get-started-with-system-manager["Démarrer avec System Manager"].
...
TIP: La console affiche ces messages « Action requise » lorsqu'un agrégat atteint 90 % de sa capacité utilisée. Si la hiérarchisation des données est activée, les messages s'affichent lorsqu'un agrégat atteint 80 % de sa capacité utilisée. Par défaut, 10 % d'espace libre sont réservés à la hiérarchisation des données. link:task-tiering.html#change-the-free-space-ratio-for-data-tiering["En savoir plus sur le ratio d'espace libre pour la hiérarchisation des données"^].
...
.. Sur la vignette agrégat, cliquez sur l’icône image:icon-action.png["Une icône représentant trois points horizontaux pour des actions supplémentaires"] puis sur *Afficher les détails de l’agrégat*.
...
+
Pour plus d'informations, veuillez consulter link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html["Réplication des données entre les systèmes"^].
...
+
Pour plus d'informations, veuillez consulter link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html#managing-data-replication-schedules-and-relationships["Gestion des planifications et des relations de réplication des données"^].
...
.. Sur la tuile d’agrégat requise, cliquez sur l’icône image:icon-action.png["Une icône représentant trois points horizontaux pour des actions supplémentaires"] puis sur *Afficher les détails de l’agrégat* pour afficher la capacité disponible (taille provisionnée moins capacité agrégée utilisée).
+
...
+
.. Sélectionnez l’agrégat, puis cliquez sur l’icône image:icon-action.png["Une icône représentant trois points horizontaux pour des actions supplémentaires"] > *Ajouter des disques*.
