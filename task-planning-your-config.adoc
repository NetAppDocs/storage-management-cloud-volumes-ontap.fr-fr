...
NOTE: La hiérarchisation des données vers Amazon Simple Storage Service (Amazon S3) n'est pas prise en charge si votre système Cloud Volumes ONTAP se trouve dans une zone locale AWS, car l'accès aux compartiments Amazon S3 en dehors de la zone locale implique une latence plus élevée et impacte les activités de Cloud Volumes ONTAP.
...
link:reference-configs-aws.html#aws["Afficher les disques par défaut pour les données système Cloud Volumes ONTAP dans AWS"].
