...
summary: Si vous avez besoin de plus de stockage après le lancement de votre système Cloud Volumes ONTAP initial, vous pouvez créer de nouveaux volumes FlexVol pour NFS, CIFS ou iSCSI à partir de la console NetApp . 
---
...
[role="lead"]
Si vous avez besoin de plus de stockage après le lancement de votre système Cloud Volumes ONTAP initial, vous pouvez créer de nouveaux volumes FlexVol pour NFS, CIFS ou iSCSI à partir de la console NetApp .
...
Quelques notes sur le provisionnement des volumes :
...
.. Sélectionnez *Ajouter un volume*.image:screenshot_add_volume_button_agg.png["Une capture d’écran du bouton Ajouter un volume sous l’onglet Agrégats."]
