...
* Si un agrégat atteint le seuil de capacité et qu'il ne peut plus prendre en charge de disques supplémentaires, la NetApp Console déplace automatiquement un volume de cet agrégat vers un agrégat disposant de capacité disponible ou vers un nouvel agrégat.
+
Si la NetApp Console crée un nouvel agrégat pour le volume, elle choisit une taille de disque adaptée à la taille de ce volume.

+
Notez que de l'espace libre est désormais disponible sur l'agrégat d'origine. Des volumes existants ou de nouveaux volumes peuvent utiliser cet espace. Cet espace ne peut pas être restitué au fournisseur de cloud dans ce scénario.
...
Si l'administrateur de l'organisation ou du compte définit le *mode de gestion des capacités* sur manuel, la console vous informe de prendre les mesures appropriées pour les décisions relatives aux capacités. Les mêmes exemples décrits dans le mode automatique s'appliquent au mode manuel, mais c'est à vous d'accepter les actions.

.Liens connexes
* link:task-manage-capacity-settings.html["Apprenez à modifier le mode de gestion de la capacité"] .
* Pour plus d'informations sur la mise à disposition des données dans AWS, veuillez consulter link:reference-configs-aws.html["Provisionnement de disques pour les agrégats dans AWS"].
