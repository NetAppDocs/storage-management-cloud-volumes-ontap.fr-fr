...
* Zone de disponibilité unique (AZ) ou plusieurs zones de disponibilité/trois zones (recommandé).
+
...
Traditionnellement, les configurations Cloud Volumes ONTAP HA dans Google Cloud utilisent un modèle non partagé, où une instance médiatrice est utilisée pour gérer les opérations de basculement ou surveiller l’état des nœuds. Cela nécessite la création d’une machine virtuelle supplémentaire dans Google Cloud et deux ensembles de disques pour chaque nœud.
...
Accès à l'instance:: Sous Debian, l'utilisateur cloud par défaut est `admin`. Google Cloud crée et ajoute un certificat pour l'utilisateur `admin` lorsque l'accès SSH est demandé via la Google Cloud Console ou la ligne de commandes gcloud. Vous pouvez spécifier `sudo` pour obtenir les privilèges root.
...
=== Disques partagés pour les configurations HA à zone de disponibilité unique

Cloud Volumes ONTAP 9.19.1 et versions ultérieures prennent en charge la configuration HA partagée pour une seule AZ. Dans l'architecture HA partagée, les deux nœuds peuvent partager un disque, ce qui modifie la méthode de création des agrégats. Au lieu de créer deux disques pour deux machines virtuelles, un seul disque est créé et attaché aux machines virtuelles. La configuration HA partagée ne nécessite pas de miroir de disque.
...
Plusieurs zones de disponibilité (recommandées):: Le déploiement d’une configuration HA sur trois zones garantit une disponibilité continue des données en cas de panne dans une zone.  Notez que les performances d'écriture sont légèrement inférieures à celles de l'utilisation d'une seule zone, mais elles sont minimes.
Zone de disponibilité unique:: Lorsqu'elle est déployée dans une seule zone de disponibilité (AZ), une configuration Cloud Volumes ONTAP HA utilise une stratégie de répartition. Cette stratégie garantit que la configuration HA est protégée contre un point de défaillance unique au sein de la zone, sans qu'il soit nécessaire d'utiliser des zones distinctes pour obtenir une isolation des pannes.
+
--
