...
* Pour AWS, vous aurez besoin d'un point de terminaison VPC vers Amazon Simple Storage Service (Amazon S3). <<Requirements to tier cold data to AWS S3,Apprendre encore plus>>.
...
+
** Version 9.4 avec les systèmes à nœud unique
...
[[requirements-to-tier-cold-data-to-amazon-s3]]
=== Exigences pour le transfert des données froides vers Amazon S3

Assurez-vous que Cloud Volumes ONTAP dispose d'une connexion à Amazon S3. La meilleure façon de fournir cette connexion est de créer un point de terminaison VPC vers le service S3. Pour obtenir des instructions, consultez le https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpce-gateway.html#create-gateway-endpoint["Documentation AWS : Création d'un point de terminaison de passerelle"^].
...
----
Le rôle personnalisé inclut les autorisations par défaut. https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["Afficher l'autorisation Azure pour l'agent de la console"^]
...
+
Si vous ne sélectionnez pas de compte de service lors du déploiement, vous devrez arrêter Cloud Volumes ONTAP, accéder à la console Google Cloud, puis associer le compte de service aux instances Cloud Volumes ONTAP. Vous pourrez ensuite activer la hiérarchisation des données comme décrit dans la section suivante.
...
+
Pour obtenir de l'aide concernant la réplication des données, veuillez vous référer à https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html["Réplication des données vers et depuis le cloud"^].
...
. Cliquez sur l’icône image:icon-action.png["Une icône représentant trois points horizontaux pour des actions supplémentaires"] pour l’agent de Console qui gère votre système Cloud Volumes ONTAP.
