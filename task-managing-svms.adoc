...
---
= Machines virtuelles de stockage Cloud Volumes ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Une machine virtuelle de stockage (SVM) est une machine virtuelle exécutée au sein d'ONTAP qui fournit des services de stockage et de données à vos clients. Vous pouvez également connaître cela sous le nom de _SVM_ ou de _vserver_. Cloud Volumes ONTAP est configuré par défaut avec une seule machine virtuelle de stockage, mais certaines configurations prennent en charge des machines virtuelles de stockage supplémentaires.
