...
Lorsque vous activez la hiérarchisation des données dans AWS, Cloud Volumes ONTAP utilise EBS comme niveau de performance pour les données actives et Amazon Simple Storage Service (Amazon S3) comme niveau de capacité pour les données inactives.
...
Lorsque vous activez la hiérarchisation des données dans Google Cloud, Cloud Volumes ONTAP utilise des disques persistants comme niveau de performance pour les données fréquemment utilisées et un bucket Google Cloud Storage comme niveau de capacité pour les données inactives. Vous pouvez sélectionner un niveau de stockage lors de la création d’un système et le modifier à tout moment par la suite. Pour des informations sur la modification de la classe de stockage, consultez link:task-tiering.html["Hiérarchiser les données inactives vers un stockage d'objets à faible coût"].
...
--
Classes de stockage:: Google Cloud utilise  https://cloud.google.com/storage/docs/storage-classes["classes de stockage"^] pour gérer les coûts et la disponibilité des données. Nous vous recommandons de sélectionner une classe de stockage en fonction de la fréquence d'accès aux données prévue, car l'accès aux données engendre des coûts plus élevés. La classe de stockage par défaut pour les données hiérarchisées est _Standard Storage_. Sauf modification manuelle, toutes les données inactives conservent la classe de stockage par défaut. Pour modifier la classe de stockage des données inactives, consultez  https://docs.cloud.google.com/storage/docs/managing-lifecycles["Documentation Google Cloud : gestion du cycle de vie des objets"^].
+
--
La classe de stockage pour la hiérarchisation des données s'applique à l'ensemble du système, et non par volume.
