...
Disques:: Les données client résident sur les blobs de page de stockage Premium. Chaque nœud a accès au stockage de l'autre nœud. Un stockage supplémentaire est également requis pour link:reference-configs-azure.html#azure-ha-pair["données de démarrage, racine et de base"].
Comptes de stockage::
+
--
...
NOTE: À compter de Cloud Volumes ONTAP 9.15.0P1, les blobs de pages Azure ne sont plus pris en charge pour les nouveaux déploiements de paires haute disponibilité. Si vous utilisez actuellement des blobs de pages Azure dans des déploiements de paires haute disponibilité existants, vous pouvez migrer vers des types d'instances de machine virtuelle plus récents dans les machines virtuelles des séries Edsv4 et Edsv5. link:reference-configs-azure.html#ha-pairs["En savoir plus sur les configurations prises en charge dans Azure"].
...
NOTE: Si vous choisissez des disques SSD gérés Premium pour un environnement qui remplit les critères ci-dessus, la console déploie automatiquement des disques SSD gérés Premium v2. Vous ne pouvez pas passer aux disques gérés Premium SSD v1.

Disques:: Les données client résident sur des disques gérés par stockage localement redondant (LRS). Chaque nœud a accès au stockage de l'autre nœud. Un stockage supplémentaire est également nécessaire pour link:reference-configs-azure.html#azure-ha-pair["données de démarrage, racine, racine partenaire, cœur et NVRAM"].
...
Disques:: Les données client résident sur des disques gérés par un système de stockage redondant de zone (ZRS). Chaque nœud a accès au stockage de l'autre nœud. Un stockage supplémentaire est également nécessaire pour link:reference-configs-azure.html#azure-ha-pair["données de démarrage, racine, racine partenaire et données de base"].
Comptes de stockage:: Les comptes de stockage sont utilisés pour les déploiements basés sur des disques gérés afin de gérer les journaux de diagnostic et la hiérarchisation du stockage blob.
...
NOTE: Cloud Volumes ONTAP utilise des équilibreurs de charge en mode actif-passif pour les sondes. Dans Azure, seul le nœud actif répond à une sonde. Le nœud passif peut apparaître comme inactif avec une croix. Il s'agit d'un comportement attendu et il n'y a pas lieu de s'inquiéter.

.Lien connexe
link:task-azure-high-availability-mode.html["Activer le mode haute disponibilité pour Cloud Volumes ONTAP dans Azure"^]
