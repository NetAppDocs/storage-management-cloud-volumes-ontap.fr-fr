...
La fonctionnalité WORM est disponible sans frais supplémentaires avec vos abonnements Marketplace et votre licence BYOL (Bring-Your-Own License). Contactez votre représentant commercial NetApp pour ajouter WORM à votre licence actuelle.
...
Vous pouvez utiliser une application pour valider des fichiers sur WORM via NFS ou CIFS, ou utiliser l'interface de ligne de commande ONTAP pour valider automatiquement des fichiers sur WORM. Vous pouvez également utiliser un fichier WORM ajoutable pour conserver des données écrites de manière incrémentielle, comme des informations de journal. Pour plus d'informations, consultez https://docs.netapp.com/us-en/ontap/snaplock/commit-files-worm-state-manual-task.html#commit-files-to-worm-manually["Transférer des fichiers vers WORM à l'aide de ONTAP SnapLock"^].

Après avoir activé le stockage WORM sur un système Cloud Volumes ONTAP, vous devez utiliser l'interface de ligne de commande ONTAP (CLI) pour toute gestion du stockage WORM. Pour obtenir des instructions, reportez-vous à la link:https://docs.netapp.com/us-en/ontap/snaplock/index.html["Documentation ONTAP sur SnapLock"^].
...
* link:concept-licensing-charging.html["Gestion des licences et facturation dans Cloud Volumes ONTAP"]
