...
Lors du déploiement de Cloud Volumes ONTAP, le champ proxy du médiateur AWS accepte une adresse de serveur. Si vous saisissez un domaine complet (FQDN) au lieu d'une adresse IP, la résolution du FQDN échouera, sauf si le VPC AWS est configuré avec un serveur DNS personnalisé. En effet, le DNS fourni par défaut par AWS ne résout que les enregistrements DNS publics et les hôtes internes AWS, et ne peut pas résoudre les FQDN externes tels que les serveurs proxy clients. Si ce scénario s'applique à vous, vous avez deux options :

* Ajoutez directement l'adresse IP du serveur proxy dans le champ proxy médiateur au lieu du domaine complet.
* Configurez un ensemble d’options DHCP dans votre VPC incluant vos adresses de serveur DNS personnalisées. Cela permet à AWS de résoudre le domaine complet (FQDN) lorsque vous le saisissez dans le champ du serveur proxy lors du déploiement. Consultez la  https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html["Documentation AWS"^] pour plus de détails.
...
TIP: Vous recherchez des informations sur l'agent Console ? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-aws.html["Afficher les règles du groupe de sécurité pour l'agent de la console"^]
...
Si vous souhaitez utiliser EBS comme niveau de performance et Amazon S3 comme niveau de capacité, vous devez vous assurer que Cloud Volumes ONTAP dispose d'une connexion à S3. La meilleure façon de fournir cette connexion est de créer un point de terminaison VPC vers le service S3. Pour obtenir des instructions, consultez le https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpce-gateway.html#create-gateway-endpoint["Documentation AWS : Création d'un point de terminaison de passerelle"^].
...
* https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["Afficher les exigences réseau pour l'agent de console"^]
