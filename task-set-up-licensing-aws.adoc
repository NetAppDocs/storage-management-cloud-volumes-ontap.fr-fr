...
[[setting-preferred-billing-for-standalone-aws-listing]]
== Définition de la facturation préférée pour une liste AWS autonome

Si vous disposez d'un abonnement AWS autonome lié à votre compte AWS, vous pouvez payer Cloud Volumes ONTAP via cet abonnement. Si vous disposez également d'abonnements NetApp Intelligent Services (NIS), vous pouvez payer vos services de données via le forfait associé à l'abonnement NIS. Pour les abonnements existants, NetApp recommande de passer à un abonnement autonome afin d'optimiser la gestion des coûts de votre utilisation de Cloud Volumes ONTAP.

.À propos de cette tâche
Si vous êtes déjà client et disposez d'abonnements NIS associés à votre compte AWS, NetApp recommande de passer à l'approche d'abonnement autonome pour une meilleure gestion des coûts de votre utilisation de Cloud Volumes ONTAP. Voici la procédure complète :
...
. (Le cas échéant) Convertissez votre forfait Professionnel en forfait Essentiel et définissez la préférence de facturation. Consultez  https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-capacity-licenses.html#change-charging-methods["Modifier les méthodes de facturation des licences de capacité"^] pour obtenir des informations sur la conversion de votre forfait Professionnel en forfait Essentiel.
. Continuez à facturer les services de données via votre abonnement NIS.


.Étapes
. Dans le panneau de gauche de la NetApp Console, sélectionnez *Administration > Licenses and subscriptions > Préférences de facturation > Cloud Volumes ONTAP*.
. Dans l'onglet AWS, sélectionnez l'abonnement autonome et cliquez sur l'icône des options image:screenshot_gallery_options.gif["icône de brochette"].
. Dans la fenêtre Modifier la configuration, sélectionnez le numéro d'abonnement de l'annonce autonome dans le champ *Informations d'identification du compte AWS*.
. Cliquez sur *Configurer*. Vous pouvez voir l'abonnement comme l'option privilégiée pour la facturation de votre Cloud Volumes ONTAP dans AWS.
. Lors du déploiement d'un nouveau système Cloud Volumes ONTAP, sélectionnez le pack Essentials pour une facturation via l'abonnement autonome. Pour plus d'informations, consultez https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-quick-deploy-aws.html["Déployez Cloud Volumes ONTAP avec Quick create dans AWS"^].


Répétez les mêmes étapes pour l'abonnement NIS dans l'onglet *Services de données* afin d'utiliser l'abonnement NIS pour vos services de données. Lors du déploiement d'un nouveau système Cloud Volumes ONTAP, sélectionnez le forfait Professional pour une facturation via l'abonnement NIS.
...
* Une licence achetée auprès de NetApp (Bring-Your-Own License ou BYOL)
...
.Étapes
. https://www.netapp.com/forms/sales-contact["Contactez le service commercial NetApp pour obtenir une licence"^]
. https://docs.netapp.com/us-en/console-setup-admin/task-adding-nss-accounts.html#add-an-nss-account["Ajoutez votre compte de site de support NetApp à la console"^]
+
...
+
Votre licence doit être disponible depuis la Console avant de pouvoir l'utiliser avec Cloud Volumes ONTAP. Si nécessaire, vous pouvez link:https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-data-services-licenses.html#add-a-license["ajouter manuellement la licence à la console"^].
...
TIP: Vous pouvez gérer les abonnements AWS Marketplace associés à vos comptes AWS depuis la page Paramètres > Informations d'identification. https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html["Apprenez à gérer vos comptes et abonnements AWS"^]
...
Choisissez l'offre Freemium pour utiliser Cloud Volumes ONTAP gratuitement avec jusqu'à 500 Gio de capacité provisionnée. link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#free-trials["En savoir plus sur l'offre Freemium"^].
...
.. Sur la page *Détails et informations d'identification*, cliquez sur *Modifier les informations d'identification > Ajouter un abonnement*, puis suivez les invites pour vous abonner à l'offre de paiement à l'utilisation sur AWS Marketplace.
+
Vous ne serez pas facturé via l'abonnement au marketplace sauf si vous dépassez 500 Gio de capacité provisionnée, auquel cas le système est automatiquement converti en link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#packages["Forfait Essentiel"^].
...
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#end-of-availability-for-node-based-licenses["Fin de disponibilité des licences basées sur des nœuds"^]
