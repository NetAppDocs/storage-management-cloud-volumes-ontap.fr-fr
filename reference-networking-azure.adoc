...
Pour plus d'informations sur la configuration des serveurs proxy, reportez-vous à la https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["Configurer l'agent de console pour utiliser un serveur proxy"^].
...
Cloud Volumes ONTAP prend en charge  https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-encryption-overview["chiffrement Azure Virtual Network (VNet)"^] le chiffrement du trafic entre machines virtuelles au sein d'un VNet ou entre VNets appariés. Cette fonctionnalité est configurée au niveau du VNet Azure et est indépendante de la topologie Cloud Volumes ONTAP (nœud unique ou HA).
...
https://learn.microsoft.com/en-us/azure/virtual-network/accelerated-networking-overview?tabs=NetworkManager["Documentation Azure : chiffrement VNet et mise en réseau accélérée"^].
...
La NetApp Console crée des groupes de sécurité Azure qui incluent les règles entrantes et sortantes permettant à Cloud Volumes ONTAP de fonctionner correctement. https://docs.netapp.com/us-en/console-setup-admin/reference-ports-azure.html["Afficher les règles du groupe de sécurité pour l'agent de la console"^]
...
Nous déconseillons de modifier les groupes de sécurité prédéfinis ou d'utiliser des groupes de sécurité personnalisés. Toutefois, si vous devez le faire, veuillez noter que le processus de déploiement exige que le système Cloud Volumes ONTAP dispose d'un accès complet à son propre sous-réseau. Une fois le déploiement terminé, si vous décidez de modifier le groupe de sécurité réseau, veillez à laisser ouverts les ports du cluster et les ports réseau HA, car la mise en réseau HA repose sur une communication sans restriction entre les deux nœuds. De plus, les adresses IP des nœuds sont définies automatiquement lors du déploiement. Ceci garantit une communication transparente au sein du cluster Cloud Volumes ONTAP (communication de type « tout à tout » entre les nœuds).
...
NOTE: Les systèmes à haute disponibilité (HA) comportent moins de règles entrantes que les systèmes à nœud unique, car le trafic de données entrant transite par l'équilibreur de charge standard Azure. Pour cette raison, le trafic provenant de l'équilibreur de charge doit être autorisé, comme indiqué dans la règle « AllowAzureLoadBalancerInBound ».
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["Afficher les exigences réseau pour l'agent de console"^]
