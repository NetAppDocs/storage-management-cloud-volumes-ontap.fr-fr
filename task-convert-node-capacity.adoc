...
* L'outil de conversion de licence prend en charge tous les types de déploiement, tels que le nœud unique, la haute disponibilité (HA) dans une seule zone de disponibilité (AZ), la haute disponibilité (HA) dans plusieurs AZ, le Bring-Your-Own License (BYOL) et le PAYGO.
...
== Conversion dans un déploiement en mode privé

Si vous disposez d'un déploiement en mode privé, vous pouvez convertir votre licence par nœud en une licence basée sur la capacité à l'aide de l'outil de conversion de licences dans la NetApp Console. La conversion est prise en charge pour tous les fournisseurs de cloud, AWS, Azure et Google Cloud.

Les abonnements au marketplace n'étant pas pris en charge en mode privé, vous devez acheter un pack Essentials Bring-Your-Own License ou BYOL auprès de NetApp. Cela vous permet de conserver les avantages d'une licence basée sur la capacité dans le modèle BYOL, sans nécessiter d'abonnement au marketplace.
