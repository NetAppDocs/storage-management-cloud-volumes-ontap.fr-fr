---
sidebar: sidebar 
permalink: whats-new.html 
keywords: 'what"s new, features, new, release notes, bugs, limitations, known issues, enhancements, fixes, cloud manager, BlueXP, digital wallet, cloud backup, cloud data sense, cloud tiering, keystone flex, keystone' 
summary: Découvrez les nouvelles fonctionnalités et améliorations de la gestion de Cloud Volumes ONTAP. 
---
= Quoi de neuf dans Cloud Volumes ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
Découvrez les nouveautés de la gestion Cloud Volumes ONTAP dans la NetApp Console.

Les améliorations décrites sur cette page sont spécifiques à la gestion de Cloud Volumes ONTAP via la console.  Pour découvrir les nouveautés du logiciel Cloud Volumes ONTAP lui-même, https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/index.html["Accédez aux notes de publication de Cloud Volumes ONTAP"^] .



[[21-september-2026]]
== 21 septembre 2026



[[support-for-capacity-based-licenses-in-private-mode-deployments]]
=== Prise en charge des licences basées sur la capacité dans les déploiements en mode privé

Vous pouvez désormais déployer des systèmes Cloud Volumes ONTAP en mode privé avec des licences basées sur la capacité sur AWS, Azure et Google Cloud.

NetApp prend en charge uniquement le pack Essentials pour les licences basées sur la capacité en mode privé. Achetez un pack Essentials Bring-Your-Own License (BYOL) basé sur la capacité auprès de NetApp et attribuez-le à votre système lors du déploiement. Les 500 premiers Gio de capacité consommée ne sont pas pris en compte dans les frais de licence. Vous pouvez acheter des packs supplémentaires à mesure que vos besoins en charge de travail augmentent.

NetApp recommande une licence basée sur la capacité pour tous les déploiements Cloud Volumes ONTAP en mode privé. Si vous disposez déjà de déploiements en mode privé avec des licences par nœud, vous pouvez les convertir en licences basées sur la capacité en achetant un pack Essentials auprès de NetApp. Cette nouvelle capacité de déploiement, combinée à l’option de conversion, soutient le plan de NetApp visant à passer progressivement des licences par nœud aux licences basées sur la capacité.

.Liens connexes
* https://review.docs.netapp.com/us-en/storage-management-cloud-volumes-ontap_capacity-license-darkmode/concept-licensing.html["Licences Cloud Volumes ONTAP"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-license-management.html["Découvrez Cloud Volumes ONTAP BYOL"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#end-of-availability-of-node-based-licenses["Fin de disponibilité des licences basées sur des nœuds"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-node-licenses.html["Gérer les licences par nœud pour Cloud Volumes ONTAP"^]




[[02-september-2026]]
== 02 septembre 2026



[[cloud-volumes-ontap-support-in-azure-secret-and-top-secret-regions-private-preview]]
=== Prise en charge de Cloud Volumes ONTAP dans les régions Azure Secret et Top Secret (aperçu privé)

Cloud Volumes ONTAP prend désormais en charge le déploiement dans les régions Azure Government Secret et Top Secret en préversion privée. Cette fonctionnalité étend Cloud Volumes ONTAP aux environnements Azure classifiés de Microsoft, permettant ainsi aux clients gouvernementaux et de défense d'utiliser ONTAP pour la gestion des données, la protection et la reprise après sinistre, afin de prendre en charge les charges de travail liées à la sécurité nationale. Contactez votre équipe NetApp pour participer.

Pour plus d'informations, consultez la documentation Azure :

* https://azure.microsoft.com/en-us/blog/azure-government-top-secret-now-generally-available-for-us-national-security-missions/["Cloud pour les missions du gouvernement américain^"]
* https://azure.microsoft.com/en-us/explore/global-infrastructure/government/national-security["Azure Government Secret"^]
* https://azure.microsoft.com/en-us/blog/azure-government-top-secret-now-generally-available-for-us-national-security-missions["Azure Government Top Secret"^]




[[25-august-2026]]
== 25 août 2026



[[support-for-austria-east-and-belgium-central-regions-in-azure]]
=== Prise en charge des régions Austria East et Belgium Central dans Azure

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures dans les régions Austria East (austriaeast) et Belgium Central (belgiumcentral) dans Azure. Les types de déploiement suivants sont pris en charge :

* Nœud unique
* Haute disponibilité dans une seule zone de disponibilité utilisant un stockage localement redondant (LRS)
* HA dans plusieurs zones de disponibilité utilisant un stockage redondant de zone (ZRS)


Pour une liste de toutes les régions prises en charge, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[10-august-2026]]
== 10 août 2026



[[cloud-volumes-ontap-9-19-1]]
=== Cloud Volumes ONTAP 9.19.1

Vous pouvez désormais utiliser la NetApp Console pour déployer et gérer la version General Availability (GA) de Cloud Volumes ONTAP 9.19.1 sur Amazon Web Service (AWS) et Google Cloud. Cette version n'est pas disponible pour le déploiement et la mise à niveau sur Microsoft Azure (Azure).

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/["En savoir plus sur cette version de Cloud Volumes ONTAP"^].



[[ability-to-convert-node-based-licenses-to-capacity-based-licensing-in-private-mode]]
=== Possibilité de convertir les licences basées sur les nœuds en licences basées sur la capacité en mode privé

NetApp a le plaisir d’annoncer la licence basée sur la capacité pour les déploiements Cloud Volumes ONTAP en mode privé. Vous pouvez désormais optimiser les coûts de vos déploiements Cloud Volumes ONTAP en mode privé en convertissant votre licence par nœud en une licence basée sur la capacité. Pour vos déploiements en mode privé, vous pouvez acheter un pack Essentials (Bring-Your-Own License ou BYOL) directement auprès de NetApp avec une capacité similaire à celle de votre licence par nœud actuelle.

Cette option de conversion prend en charge le plan progressif de NetApp visant à abandonner les licences par nœud au profit des licences basées sur la capacité. Contrairement aux licences par nœud, qui allouent une capacité fixe à chaque système, les licences basées sur la capacité puisent dans un pool de capacité partagé entre vos systèmes Cloud Volumes ONTAP, vous offrant ainsi une plus grande flexibilité pour redistribuer les charges de travail à mesure que vos besoins de stockage évoluent.

.Liens connexes
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#end-of-availability-of-node-based-licenses["Fin de disponibilité des licences basées sur des nœuds"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-convert-node-capacity.html#conversion-in-a-private-mode-deployment["Conversion en mode de déploiement privé"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-node-licenses.html["Gérer les licences par nœud pour Cloud Volumes ONTAP"^]




[[03-august-2026]]
== 03 août 2026



[[cloud-volumes-ontap-integration-with-microsoft-onelake]]
=== Intégration de Cloud Volumes ONTAP avec Microsoft OneLake

Cloud Volumes ONTAP prend désormais en charge l'intégration avec link:https://learn.microsoft.com/en-us/fabric/onelake/onelake-overview["Microsoft OneLake"^] en préversion publique. Vous pouvez intégrer Cloud Volumes ONTAP avec link:https://community.fabric.microsoft.com/["Microsoft Fabric"^] en utilisant link:https://learn.microsoft.com/en-us/fabric/onelake/onelake-overview["Microsoft OneLake"^] pour rendre les données NAS Cloud Volumes ONTAP existantes accessibles aux services d'analyse et d'IA de Microsoft, tels que Power BI, Azure AI Search, OpenAI et Microsoft Copilot. Cette intégration ne nécessite aucun déplacement des données ni duplication. L'intégration utilise un accès link:https://docs.netapp.com/us-en/ontap/s3-multiprotocol/index.html["ONTAP S3 multiprotocole"^] pour connecter Cloud Volumes ONTAP à un Microsoft Fabric lakehouse existant.

.Articles et ressources connexes :
* https://www.netapp.com/blog/optimize-ai-analytics-microsoft-onelake/?utm_content=nativelink&utm_medium=internal&utm_source=linkedin&utm_campaign=b2d-port-all-na-ww-digi-blog-1747859361262&es_id=956c739202["NetApp blog : Optimisez l’IA et l’analytique avec Microsoft OneLake et NetApp Cloud Volumes ONTAP"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-azure-onelake.html["Intégrez les systèmes Cloud Volumes ONTAP avec Microsoft OneLake pour l’IA et l’analytique"^]
* https://www.netapp.com/media/160786-na-1263-netapp-cloud-volumes-ontap-azure-onelake.pdf["Intégration de Cloud Volumes ONTAP avec Microsoft OneLake - Note technique"^]




[[fix-dismiss-and-reactivate-configurations-for-cloud-volumes-ontap-in-microsoft-azure]]
=== Corrigez, ignorez et réactivez les configurations pour Cloud Volumes ONTAP dans Microsoft Azure

Le tableau de bord bien conçu dans la NetApp Console vous permet désormais de corriger les recommandations de configuration pour vos systèmes Cloud Volumes ONTAP.

Vous pouvez également exclure une ou plusieurs configurations de l'analyse d'architecture. Cela vous permet d'ignorer les configurations qui ne s'appliquent pas à votre environnement.

Si vous souhaitez réintégrer ultérieurement une configuration rejetée, vous pouvez la réactiver dans le tableau de bord bien architecturé. Cela rétablit la configuration dans l'analyse et vous permet d'obtenir des recommandations et des correctifs pour celle-ci.

* link:https://docs.netapp.com/us-en/console-well-architected/cvo/improve-configurations-cvo.html["Corrigez, ignorez et réactivez les configurations pour Cloud Volumes ONTAP dans Microsoft Azure"^]




[[23-july-2026]]
== 23 juillet 2026



[[support-for-chile-central-region-in-azure]]
=== Prise en charge de la région Chile Central dans Azure

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures dans la région Chile Central (chilecentral) dans Azure. Les types de déploiement suivants sont pris en charge :

* Nœud unique
* Haute disponibilité dans une seule zone de disponibilité utilisant un stockage localement redondant (LRS)
* HA dans plusieurs zones de disponibilité utilisant un stockage redondant de zone (ZRS)


Pour une liste de toutes les régions prises en charge, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[20-july-2026]]
== 20 juillet 2026



[[introducing-simplified-onboarding-of-cloud-volumes-ontap-from-aws-marketplace]]
=== Présentation de l'intégration simplifiée de Cloud Volumes ONTAP depuis AWS Marketplace

NetApp introduit un flux d'intégration simplifié pour Cloud Volumes ONTAP depuis AWS Marketplace, réduisant les étapes manuelles et les frictions liées à l'intégration.

Auparavant, le déploiement Cloud Volumes ONTAP depuis AWS Marketplace nécessitait de naviguer entre plusieurs écrans et de configurer manuellement les rôles, les politiques et les informations d'identification. Cela rendait la prise en main lente et sujette aux erreurs.

Le nouveau processus d'intégration simplifie votre transition d'un abonnement à la place de marché vers un environnement Cloud Volumes ONTAP entièrement opérationnel grâce aux fonctionnalités suivantes :

. Abonnement à la fiche dédiée Cloud Volumes ONTAP sur AWS Marketplace : vous vous abonnez à la fiche autonome Cloud Volumes ONTAP sur AWS Marketplace, achetez un abonnement et configurez votre compte AWS (si vous n’en avez pas déjà un).
. Parcours guidé sur la NetApp Console : Vous configurez votre compte dans la NetApp Console, et la Console vous accompagne automatiquement tout au long du processus de déploiement en affichant le parcours guidé sur le côté droit. Si vous devez quitter la Console, vous pouvez reprendre le parcours guidé ultérieurement.
. Association de votre abonnement : Vous pouvez associer automatiquement votre abonnement à la Console, évitant ainsi les étapes manuelles de liaison.
. Déploiement de l'agent Console avec configuration des identifiants AWS : vous pouvez créer automatiquement les rôles AWS requis. Le processus déploie une pile CloudFormation dans votre compte AWS avec les rôles et politiques nécessaires pour que vous puissiez les examiner, puis les associe à la NetApp Console, éliminant ainsi la configuration manuelle d'IAM. Vous poursuivez ensuite la configuration de l’agent, du réseau et des paramètres du groupe de sécurité pour commencer le déploiement de l’agent.
. Déployez un nouveau système Cloud Volumes ONTAP sur AWS : vous accédez directement à l’ `Quick create`assistant pour un déploiement Cloud Volumes ONTAP plus rapide et simplifié.


.Liens connexes
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-onboard-cvo-aws-marketplace.html["Intégrez Cloud Volumes ONTAP depuis AWS Marketplace"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-quick-deploy-aws.html["Déployez Cloud Volumes ONTAP avec Quick create dans AWS"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#flexible-billing-for-cloud-volumes-ontap-and-data-services-in-aws["Facturation flexible pour Cloud Volumes ONTAP et les services de données dans AWS"^]




[[well-architected-configuration-analysis-for-cloud-volumes-ontap-in-azure]]
=== Analyse de configuration bien architecturée pour Cloud Volumes ONTAP dans Azure

La NetApp Console prend désormais en charge l'analyse de configuration bien architecturée pour les systèmes Cloud Volumes ONTAP exécutés dans Microsoft Azure. La Console analyse quotidiennement vos déploiements et fournit un état bien architecturé, des informations et des recommandations pour vous aider à maintenir des systèmes alignés sur les meilleures pratiques NetApp et le cadre bien architecturé.

Le tableau de bord bien architecturé affiche le nombre total de systèmes analysés et catégorise leurs ressources par configuration et niveau de gravité. Si vous êtes administrateur système, vous pouvez consulter les recommandations pour tout problème de configuration au sein de votre organisation directement depuis le tableau de bord de la Console et télécharger un rapport CSV de toutes les recommandations. Les configurations sont catégorisées selon :

* *Fiabilité* : Garantit le bon fonctionnement et la constance des charges de travail, même en cas de perturbations.
* *Sécurité* : Protège les données, les systèmes et les actifs grâce à des évaluations des risques et à leur atténuation.
* *Excellence opérationnelle* : Optimise l'architecture pour maximiser la valeur commerciale.
* *Optimisation des coûts* : Réduit et optimise les coûts pour une plus grande valeur commerciale.


.Liens connexes
* https://docs.netapp.com/us-en/console-well-architected/cvo/configuration-analysis-cvo.html["Analyse de configuration pour Cloud Volumes ONTAP"^]
* https://docs.netapp.com/us-en/console-well-architected/cvo/improve-configurations-cvo.html["Mettre en œuvre des configurations système Cloud Volumes ONTAP bien architecturées"^]




[[14-july-2026]]
== 14 juillet 2026



[[cloud-volumes-ontap-9-19-1]]
=== Cloud Volumes ONTAP 9.19.1

La version Release Candidate (RC) de Cloud Volumes ONTAP 9.19.1 est disponible uniquement sur Google Cloud pour la mise à niveau vers la version 9.19.1 General Availability (GA).

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/["En savoir plus sur cette version de Cloud Volumes ONTAP"^].



[[support-for-next-generation-google-cloud-vms-in-cloud-volumes-ontap-ha]]
=== Prise en charge des machines virtuelles Google Cloud de nouvelle génération dans Cloud Volumes ONTAP HA

Avec la version 9.19.1, NetApp fait passer les déploiements Cloud Volumes ONTAP à haute disponibilité (HA) des machines virtuelles N2 aux machines virtuelles de nouvelle génération de la série Google Cloud C3 pour une expérience plus rapide et plus évolutive.


NOTE: Cette assistance s'applique uniquement aux déploiements HA à zone de disponibilité unique (AZ).

Vous pouvez désormais tirer parti des machines virtuelles de la série C3 lors du déploiement de Cloud Volumes ONTAP 9.19.1 et versions ultérieures sur Google Cloud. Ces machines offrent des performances améliorées et des limites de capacité plus élevées. Elles utilisent Google Virtual NIC (gVNIC) et des disques Hyperdisk Balanced pour des performances dynamiques sur les charges de travail intensives.

Si votre système Cloud Volumes ONTAP exécute la version 9.19.1 ou ultérieure, les packages préconfigurés que vous utilisez pour des déploiements simplifiés dans une seule zone de disponibilité utilisent automatiquement des machines virtuelles C3, tout en vous permettant de personnaliser les paramètres d'IOPS et de débit en fonction des besoins de votre charge de travail. De même, lors de la création d'agrégats, vous pouvez ajouter des disques Zonal Hyperdisk Balanced pour obtenir de meilleures performances et une meilleure évolutivité sur Google Cloud. Vous pouvez également sélectionner les variantes LSSD des machines de la série C3 pour une prise en charge par défaut du Flash Cache.

Notez ces points :

* Dans vos déploiements Cloud Volumes ONTAP, vous ne pouvez pas changer des machines virtuelles C3 en machines virtuelles de la série N2 et inversement. Cependant, vous pouvez changer de type d'instance parmi les différentes variantes de la famille C3.
+

NOTE: Le changement de type d'instance au sein de la famille C3 peut prendre plus de temps en fonction du nombre de disques de données connectés. Planifiez votre configuration à l'avance pour éviter tout impact sur les performances.

* Lors du changement de type d'instance, si vous sélectionnez c3-standard-44 ou c3-standard-88 et leurs variantes SSD locales (LSSD) comme machine virtuelle cible, le niveau de réseau de sortie du service réseau Google _Tier-1_ est appliqué automatiquement. Si vous revenez à une autre variante C3, le niveau de réseau de sortie est défini sur _Standard_.
+

NOTE: Le passage au niveau _Tier-1_ du réseau de sortie engendre des coûts supplémentaires. Prévoyez le déploiement et la mise à niveau en conséquence.

* Si vous sélectionnez les variantes LSSD de C3, le cache Flash est activé par défaut. Vous ne pouvez pas changer de variante LSSD de C3 dans votre déploiement Cloud Volumes ONTAP, même si la variante cible prend en charge le cache Flash.
* Sur vos machines virtuelles C3, vous pouvez ajouter des volumes aux agrégats nouveaux et existants.
* Lors de la réplication d'un système Cloud Volumes ONTAP avec un type de VM N2 vers un type de VM C3, le type de disque est défini par défaut sur Hyperdisk Balanced.
* Vous pouvez modifier les paramètres IOPS et de débit lors de l’ajout d’agrégats via la NetApp Console ou en utilisant l’ `PUT /gcp/ha/aggregates/{workingEnvironmentId}/{aggregateName}` API.


https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-gcp.html["Configurations prises en charge pour Cloud Volumes ONTAP dans Google Cloud"^]

https://docs.cloud.google.com/compute/docs/general-purpose-machines#c3_series["Google documents : série de machines C3"^]



[[simplified-cloud-volumes-ontap-deployment-in-google-cloud]]
=== Déploiement simplifié de Cloud Volumes ONTAP dans Google Cloud

Vous pouvez désormais utiliser l'assistant *Quick create* pour déployer Cloud Volumes ONTAP sur Google Cloud, aussi bien pour les configurations à nœud unique que pour les configurations à haute disponibilité (paire haute disponibilité). Ce processus simplifié réduit le nombre d'étapes, définit automatiquement les valeurs par défaut sur une seule page et minimise la navigation pour un déploiement plus rapide et plus facile.

.Liens connexes
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-quick-deploy-gcp.html["Déployez Cloud Volumes ONTAP avec Quick create dans Google Cloud"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-deploying-gcp.html["Déployez Cloud Volumes ONTAP à l'aide des options avancées dans Google Cloud"^]




[[introducing-access-roles-for-cloud-volumes-ontap]]
=== Introduction des rôles d'accès pour Cloud Volumes ONTAP

NetApp introduit des rôles de gestion des identités et des accès (IAM) pour Cloud Volumes ONTAP, permettant le contrôle d'accès basé sur les rôles (RBAC) pour une meilleure sécurité et une conformité facilitée. Les utilisateurs peuvent continuer à se connecter à la NetApp Console en utilisant leurs identifiants existants pour les opérations Cloud Volumes ONTAP. Cependant, l'authentification est désormais acheminée via la fédération d'identités, ce qui permet à l'administrateur de la Console d'exercer un contrôle plus granulaire sur ces fonctions. Chaque utilisateur doit disposer de l'un des rôles suivants pour effectuer toute opération sur Cloud Volumes ONTAP. Sans rôle, les utilisateurs ne peuvent ni consulter ni accéder aux instances Cloud Volumes ONTAP.

* *Cloud Volumes ONTAP viewer* : Accorde un accès en lecture seule aux écrans et onglets de Cloud Volumes ONTAP. Certains écrans et boutons d’action sont désactivés dans ce rôle.
* *Cloud Volumes ONTAP operator* : Permet d’effectuer les tâches de gestion quotidiennes, telles que l’ajout de volumes, la gestion du stockage et la configuration de la réplication.
* *Administrateur Cloud Volumes ONTAP* : Fournit un accès complet à toutes les opérations Cloud Volumes ONTAP.


L'administrateur de la NetApp Console attribue des rôles aux utilisateurs Cloud Volumes ONTAP depuis *Administration > Identité*. Les autorisations de rôle sont basées sur l'organisation de l'utilisateur et s'appliquent à tous les systèmes Cloud Volumes ONTAP de cette organisation.

Cloud Volumes ONTAP applique le RBAC à l'aide d'un cache local des autorisations de chaque utilisateur, qui est actualisé automatiquement une fois par heure. Lorsqu'un utilisateur se voit attribuer un nouveau rôle ou que son rôle existant est modifié, la prise en compte de cette modification peut prendre jusqu'à une heure, après quoi tous les workflows autorisés par les rôles sont activés.


NOTE: Lors du premier déploiement d'un agent Console ou de la mise à niveau d'un agent Console existant vers Console 4.9.0 ou une version ultérieure, un processus d'initialisation unique attribue automatiquement les rôles Cloud Volumes ONTAP à tous les utilisateurs existants de l'organisation. Ce processus peut prendre jusqu'à 30 minutes pour être terminé. Une fois terminé, la Console attribue les rôles appropriés aux utilisateurs existants.

Les rôles Cloud Volumes ONTAP s'appliquent aux environnements Amazon Web Service (AWS), Microsoft Azure (Azure) et Google Cloud.

.Liens connexes
* https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["NetApp Console rôles d'accès"^]
* https://docs.netapp.com/us-en/console-setup-admin/reference-iam-cvo-roles.html["Rôles d'accès Cloud Volumes ONTAP dans la NetApp Console"^]




[[11-june-2026]]
== 11 juin 2026



[[support-for-a-new-azure-region-in-cloud-volumes-ontap]]
=== Prise en charge d'une nouvelle région Azure dans Cloud Volumes ONTAP

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures dans une ou plusieurs zones de disponibilité dans Azure dans la région Malaysia West (malaysiawest). Cela inclut la prise en charge des déploiements à nœud unique et à haute disponibilité (HA).

Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[08-june-2026]]
== 08 juin 2026



[[support-for-new-ec2-instances-for-cloud-volumes-ontap-in-aws]]
=== Prise en charge des nouvelles instances EC2 pour Cloud Volumes ONTAP dans AWS

Cloud Volumes ONTAP 9.16.1 et versions ultérieures prennent en charge les familles d'instances EC2 M6id et M6idn pour les déploiements à nœud unique et à haute disponibilité (HA) sur AWS, qu'ils soient nouveaux ou existants. Ces instances de 6e génération offrent une puissance de traitement plus rapide, des capacités réseau renforcées et des performances supérieures aux systèmes à usage général de 5e génération.

* Instances M6id : Elles offrent un équilibre entre puissance de calcul et mémoire grâce à un stockage NVMe local haute vitesse et faible latence. Toutes les tailles d’instances M6id prennent en charge le cache Flash. Ces instances sont parfaitement adaptées aux charges de travail telles que l’enregistrement de données et le traitement multimédia.
* *Instances M6idn* : Offrent les mêmes capacités de stockage NVMe local que les instances M6id. Toutes les tailles d’instances M6idn prennent en charge Flash Cache.


Nous déconseillons les instances de petite taille m6id.xlarge, m6id.2xlarge, m6idn.xlarge et m6idn.2xlarge pour les charges de travail exigeantes en termes de performances ou de mémoire. Pour ces charges de travail, privilégiez une instance dotée d'un nombre de cœurs plus élevé et d'une mémoire plus importante.

Pour plus d'informations, consultez  https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-aws.html["Configurations prises en charge pour Cloud Volumes ONTAP dans AWS"^].



[[18-may-2026]]
== 18 mai 2026



[[simplified-cloud-volumes-ontap-deployment-in-azure]]
=== Déploiement simplifié de Cloud Volumes ONTAP dans Azure

Vous pouvez désormais utiliser la création rapide pour déployer Cloud Volumes ONTAP dans Azure, aussi bien pour les configurations à nœud unique que pour les configurations à haute disponibilité (HA). Ce processus simplifié réduit le nombre d'étapes par rapport à la méthode avancée, définit automatiquement les valeurs par défaut sur une seule page et minimise la navigation, rendant le déploiement plus rapide et plus facile.

.Liens connexes
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-quick-deploy-azure.html["Déployez Cloud Volumes ONTAP avec Quick create dans Azure"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-deploying-otc-azure.html["Déployez Cloud Volumes ONTAP à l'aide des options avancées dans Azure"^]




[[12-may-2026]]
== 12 mai 2026



[[flexible-billing-for-cloud-volumes-ontap-and-data-services-in-aws]]
=== Facturation flexible pour Cloud Volumes ONTAP et les services de données dans AWS

NetApp propose des options de facturation améliorées pour vos abonnements Cloud Volumes ONTAP dans AWS, vous offrant un meilleur contrôle sur la façon dont vous payez la capacité Cloud Volumes ONTAP et les services de données NetApp, tels que NetApp Backup and Recovery. Auparavant, toute l’utilisation de Cloud Volumes ONTAP et des services de données était facturée ensemble via l’offre NetApp Intelligent Services (NIS) sur la place de marché AWS.

Suite aux modifications apportées au programme de remises pour entreprises AWS (EDP), NetApp a introduit un abonnement Cloud Volumes ONTAP autonome sur la place de marché AWS qui vous permet de dissocier la facturation de la capacité Cloud Volumes ONTAP de celle des services de données. Cette dissociation vous offre une plus grande flexibilité dans la gestion de vos crédits et coûts EDP. NetApp recommande d'utiliser l'abonnement autonome pour une meilleure gestion des coûts liés à votre utilisation de Cloud Volumes ONTAP, tout en continuant d'utiliser l'abonnement NIS pour la facturation des services de données après le 31 juillet 2026, lorsque les exigences EDP entreront en vigueur.

L'abonnement Cloud Volumes ONTAP autonome sur AWS prend uniquement en charge les packages de licence Essentials ou Freemium valides, basés sur la capacité. Le package Professional n'est pas pris en charge. Vous pouvez choisir le mode de facturation qui correspond le mieux à vos besoins :

* *Abonnement Cloud Volumes ONTAP autonome (recommandé)* : payez votre utilisation de Cloud Volumes ONTAP via l’abonnement autonome sur la place de marché AWS, tout en continuant à facturer les services de données via l’abonnement NIS. C’est l’option recommandée pour la plupart des clients qui disposent à la fois d’un abonnement autonome et d’un abonnement NIS associés à leur compte AWS dans la NetApp Console.
* *Abonnement NIS combiné* : payez à la fois Cloud Volumes ONTAP et les services de données ensemble via l'inscription NIS existante. La formule combinée sera valable pour les clients existants qui ont uniquement des abonnements NIS associés à leur compte AWS dans la NetApp Console. Cependant, NetApp recommande de définir vos préférences de facturation pour votre utilisation de Cloud Volumes ONTAP et de passer à l'abonnement autonome pour une meilleure gestion des coûts.


Selon votre type d'abonnement, vous bénéficiez des scénarios suivants :

.Abonnement autonome uniquement (nouveaux clients)
Si vous ne disposez que d'un abonnement AWS autonome associé à votre compte AWS dans la NetApp Console et d'aucun abonnement NIS, vous ne pouvez sélectionner que le pack de licences Essentials lors du déploiement. L'option de service de données *Backup and Recovery* ne vous est pas disponible.

.Abonnements autonomes et NIS (clients existants)
Si votre compte AWS comprend à la fois un abonnement NIS et un abonnement autonome, et que vous n'avez pas défini de préférence de facturation dans l'onglet *Préférences de facturation* de la NetApp Console, l'utilisation de Cloud Volumes ONTAP continue d'être facturée par défaut via NIS. Ceci est également le comportement par défaut pour toute instance Cloud Volumes ONTAP exécutant le package Professional, car l'offre autonome ne prend pas en charge Professional. Vous pouvez définir une préférence de facturation et choisir le mode de facturation de chaque service :

* Configurez l'abonnement autonome comme compte de facturation préféré pour Cloud Volumes ONTAP (avec le package Essentials). Si vous ne possédez que le package Professional, vous pouvez le convertir en package Essentials et facturer votre utilisation de Cloud Volumes ONTAP en définissant la préférence de facturation. Nous recommandons cette méthode car elle vous permet de gérer vos crédits EDP Cloud Volumes ONTAP plus efficacement.
* Utilisez l'abonnement NIS pour la facturation des services de données ou l'utilisation de Cloud Volumes ONTAP via le pack Professionnel. Lors du déploiement, activez le service de données *Backup and Recovery* et sélectionnez le pack de licences Professionnel pour facturer les services de données via l'abonnement NIS.


.Abonnement NIS uniquement (clients existants)
Ceci s'applique si vous êtes déjà client et disposez d'abonnements NIS associés à votre compte AWS. Vous pouvez continuer à facturer Cloud Volumes ONTAP et les services de données ensemble comme actuellement. Cependant, NetApp recommande de passer à l'approche d'abonnement autonome pour une meilleure gestion des coûts de votre utilisation de Cloud Volumes ONTAP :

* Abonnez-vous à une nouvelle fiche produit autonome sur le portail AWS.
* Associez l'abonnement autonome à votre compte AWS.
* Définissez votre préférence de facturation sur l’abonnement autonome pour l’utilisation de Cloud Volumes ONTAP avec le forfait Essentials.
* (Le cas échéant) Convertissez votre forfait Professional en forfait Essentials et définissez votre préférence de facturation.
* Continuez à facturer les services de données via votre abonnement NIS.



NOTE: Aucun comptage en double ne se produit lorsque vous utilisez plusieurs types d'abonnement ou que vous migrez entre des annonces.

.Lien connexe
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-set-up-licensing-aws.html["Configurer les licences pour Cloud Volumes ONTAP dans AWS"^]
* https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-capacity-licenses.html#change-charging-methods["Modifier les méthodes de facturation des licences de capacité"^]




[[13-april-2026]]
== 13 avril 2026



[[improved-cloud-volumes-ontap-support-with-google-cloud-c3-vms]]
=== Prise en charge améliorée de Cloud Volumes ONTAP avec les machines virtuelles Google Cloud C3

Cloud Volumes ONTAP prend désormais en charge de manière améliorée les machines virtuelles de la série Google Cloud C3, ce qui permet d'activer les fonctionnalités supplémentaires suivantes :

* Dans vos déploiements Cloud Volumes ONTAP, vous pouvez basculer entre différents types d'instances au sein de la famille C3.
* Lors du changement de type d'instance, si vous sélectionnez c3-standard-44 ou c3-standard-88 et leurs variantes LSSD comme machine virtuelle cible, le service réseau Google applique automatiquement le niveau de réseau de sortie Tier-1. Si vous choisissez une autre variante C3, le niveau de réseau de sortie est défini sur Standard.
+

NOTE: Le passage au niveau 1 du réseau de sortie engendre des coûts supplémentaires. Prévoyez le déploiement et la mise à niveau en conséquence.

* Si vous sélectionnez les variantes SSD locales (LSSD) de C3, le cache Flash est activé par défaut. Vous ne pouvez pas changer de variante LSSD de C3 dans votre déploiement Cloud Volumes ONTAP, même si la variante cible prend en charge le cache Flash.
* Sur vos machines virtuelles C3, vous pouvez désormais ajouter des volumes aux agrégats nouveaux et existants.
* Vous pouvez modifier les paramètres IOPS et de débit lors de l’ajout de volumes via la NetApp Console ou en utilisant l’ `PUT /gcp/vsa/aggregates/{workingEnvironmentId}/{aggregateName}` API.


.Liens connexes
* https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-gcp.html["Configurations prises en charge pour Cloud Volumes ONTAP dans Google Cloud"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#cloud-volumes-ontap-support-for-next-generation-google-cloud-vms["Prise en charge de Cloud Volumes ONTAP pour les machines virtuelles Google Cloud de nouvelle génération"^]




[[10-march-2026]]
== 10 mars 2026



[[ability-to-manage-console-agent-proxy-settings-for-cloud-volumes-ontap]]
=== Possibilité de gérer les paramètres proxy de l'agent NetApp Console pour Cloud Volumes ONTAP

Vous pouvez désormais gérer les paramètres de proxy sur l’agent NetApp Console pour Cloud Volumes ONTAP, même si vous perdez la connexion ou si le proxy est mal configuré. Auparavant, si l’agent Console ne pouvait pas se connecter à Cloud Volumes ONTAP pendant 20 minutes, il remplaçait vos paramètres de proxy manuels par les paramètres par défaut. Cela provoquait des échecs de communication, y compris des problèmes avec les messages AutoSupport. Pour conserver vos paramètres de proxy pour les systèmes existants, exécutez l’appel d’API suivant :

`PUT /occm/config`

Incluez le paramètre suivant dans le corps de la requête :

[source, json]
----
{
  "proxyMode": "No_Overwrites"
}
----
Le mode par défaut est standard, ce qui signifie que l'agent de la Console remplace vos paramètres de proxy par les valeurs par défaut s'il ne parvient pas à se connecter à Cloud Volumes ONTAP pendant 20 minutes.

https://docs.netapp.com/us-en/console-automation/cvo/put-occm-config.html["Configurer les paramètres modifiables de la NetApp Console"^]



[[26-february-2026]]
== 26 février 2026



[[support-for-google-infrastructure-manager-for-private-mode-deployments]]
=== Prise en charge de Google Infrastructure Manager pour les déploiements en mode privé

Cloud Volumes ONTAP 9.16.1 et versions ultérieures prennent désormais en charge  https://docs.cloud.google.com/infrastructure-manager/docs/overview["Google Cloud Infrastructure Manager"^] (IM) au lieu de  https://docs.cloud.google.com/deployment-manager/docs/deprecations["Cloud Deployment Manager"^] (DM) pour les nouveaux déploiements en mode privé sur Google Cloud. Google va déprécier Deployment Manager en tant que service d'infrastructure dans un avenir proche au profit d'Infrastructure Manager, plus avancé.

À compter du 25 février 2026, Cloud Volumes ONTAP utilise Infrastructure Manager pour les déploiements en mode privé, nouveaux et existants. Ce tableau explique les flux de travail de base pour vous :

[cols="1,4,1,1,1,1"]
|===
| Scénario | Action | Nouvelles API pour l'agent | Nouvelles autorisations pour l'agent | Nouvelles API Google Cloud pour Cloud Volumes ONTAP | Ressources documentaires 


| Agent existant et déploiement existant en mode privé | Mettez à niveau l'agent NetApp Console vers la dernière version en téléchargeant le programme d'installation depuis le site de support NetApp, puis installez manuellement l'agent sur votre hôte afin qu'il puisse utiliser les API d'Infrastructure Manager. Ensuite, convertissez les systèmes Cloud Volumes ONTAP existants pour utiliser Infrastructure Manager.  a| 
* API Cloud Infrastructure Manager
* API Cloud Quotas
* API Cloud Build

 a| 
Toutes les autorisations listées pour la version Console sur :

* https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#08-december-2025["8 décembre 2025"^]
* https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#09-february-2026["09 février 2026"^]
* cloudbuild.workerpools.get
* cloudbuild.workerpools.get

 a| 
* \https://cloudbuild.googleapis.com/v1
* \https://www.googleapis.com/upload/storage/v1
* \https://config.googleapis.com/v1

| https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-gcp-dm-im-conv.html["Configurer les déploiements Cloud Volumes ONTAP existants pour Google Cloud Infrastructure Manager"^] 


| Nouvel agent et nouveau déploiement | Créez un nouvel agent et déployez un nouveau système Cloud Volumes ONTAP en mode privé. |  |  |   a| 
* https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html["Créer un agent de Console à partir de Google Cloud"^]
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-private-mode.html["Démarrage rapide pour les déploiements en mode privé"^]


|===
Dans les déploiements en mode privé, vous devez effectuer quelques modifications de configuration pour que Cloud Volumes ONTAP commence à utiliser Infrastructure Manager. Consultez  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#infrastructure-manager-configuration-for-private-mode-deployments["Configuration d'Infrastructure Manager pour les déploiements en mode privé"^].

.Liens connexes
* https://docs.netapp.com/us-en/console-setup-admin/whats-new.html#console-agent-4-2-0["NetApp Console Agent 4.2.0 Notes de version"^]
* https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#gcp-permissions-change-log["Autorisations requises pour Google Cloud Infrastructure Manager"^]




[[19-february-2026]]
== 19 février 2026



[[new-regions-supported-in-azure]]
=== Nouvelles régions prises en charge dans Azure

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures dans des zones de disponibilité uniques et multiples dans Azure dans les régions suivantes. Cela inclut la prise en charge des déploiements à nœud unique et à haute disponibilité (HA).

* Japon Ouest (japanwest)
* Indonésie Central (indonesiacentral)


Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[17-february-2026]]
== 17 février 2026



[[cloud-volumes-ontap-support-for-next-generation-google-cloud-vms]]
=== Prise en charge de Cloud Volumes ONTAP pour les machines virtuelles Google Cloud de nouvelle génération

Avec la version 9.18.1, NetApp fait passer les nouveaux déploiements Cloud Volumes ONTAP des machines virtuelles N2 aux machines virtuelles de la série C3 de nouvelle génération de Google Cloud, pour une expérience plus rapide et plus évolutive. Vous pouvez désormais tirer parti des machines virtuelles de la série C3 lors du déploiement de Cloud Volumes ONTAP 9.18.1 et versions ultérieures sur Google Cloud. Les machines de la série C3 offrent des performances améliorées et des limites de capacité plus élevées grâce à l'utilisation de Google Virtual NIC (gVNIC) et de disques Hyperdisk Balanced, garantissant des performances dynamiques pour les charges de travail intensives.


NOTE: Pour le moment, Cloud Volumes ONTAP prend en charge la série C3 uniquement dans les déploiements à nœud unique.

Si votre système Cloud Volumes ONTAP exécute la version 9.18.1 ou ultérieure, les packages préconfigurés que vous utilisez pour des déploiements sur un seul nœud utilisent automatiquement des machines virtuelles C3, tout en vous permettant de personnaliser les paramètres d'IOPS et de débit en fonction des besoins de votre charge de travail. De même, lors de la création d'agrégats, vous pouvez ajouter des disques Hyperdisk Balanced pour obtenir de meilleures performances et une meilleure évolutivité dans Google Cloud. De plus, vous pouvez sélectionner les variantes LSSD des machines de la série C3 pour une prise en charge par défaut du Flash Cache.

Il est impossible de modifier le type de disque des machines virtuelles C3 lors de l'ajout de volumes aux agrégats, car C3 ne prend en charge que les disques Hyperdisk Balanced. De même, lors de la réplication d'un système avec des machines virtuelles N2 vers une machine virtuelle C3, le type de disque est défini par défaut sur Hyperdisk Balanced.

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-gcp.html["Configurations prises en charge pour Cloud Volumes ONTAP dans Google Cloud"^]

https://docs.cloud.google.com/compute/docs/general-purpose-machines#c3_series["Google documents : série de machines C3"^]



[[vnet-security-for-cloud-volumes-ontap-in-azure]]
=== Sécurité VNet pour Cloud Volumes ONTAP dans Azure

Les déploiements Cloud Volumes ONTAP 9.18.1 et ultérieurs dans les zones de disponibilité uniques et multiples d'Azure prennent en charge le chiffrement Azure Virtual Network (VNet) dans le cadre de leur stratégie de sécurité multicouche afin de protéger les données en transit. Cloud Volumes ONTAP exploite le protocole Datagram Transport Layer Security (DTLS) natif d'Azure pour sécuriser la communication entre les nœuds ONTAP, les interfaces de gestion et les autres services Azure, empêchant l'interception et l'accès non autorisé. Ce chiffrement au niveau du réseau complète les protections intégrées d'ONTAP pour le stockage et les données au repos afin d'assurer la sécurité de bout en bout de vos données.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-azure.html#networking-for-azure-vnet-encryption["Mise en réseau pour le chiffrement Azure VNet"]



[[12-february-2026]]
== 12 février 2026



[[support-for-ebdsv5-and-e104idsv5-vms-in-azure]]
=== Prise en charge des machines virtuelles Ebdsv5 et E104ids_v5 dans Azure

À partir de Cloud Volumes ONTAP 9.18.1, vous pouvez déployer des machines virtuelles Ebdsv5 et E104ids_v5 pour des déploiements et des mises à niveau à nœud unique et en paire haute disponibilité (HA).

Les machines virtuelles Ebdsv5 de la famille Eb d'Azure sont optimisées pour des performances de stockage distant supérieures. Vous pouvez utiliser ces machines virtuelles pour des charges de travail d'entreprise gourmandes en mémoire et en E/S, telles que les bases de données relationnelles, l'analyse en mémoire et d'autres applications critiques exigeantes.

E104ids_v5 est une instance de machine virtuelle isolée qui vous aide à mieux gérer les fenêtres de maintenance planifiées. Comparée à E80ids_v4, elle offre un débit disque et des IOPS nettement supérieurs, ainsi que de meilleures performances réseau globales.

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html["Configurations prises en charge pour Cloud Volumes ONTAP dans Azure"^]

https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/memory-optimized/edsv5-series?tabs=sizebasic["Documentation Azure : série de tailles Edsv5"^]



[[10-february-2026]]
== 10 février 2026



[[cloud-volumes-ontap-9-18-1-ga]]
=== Cloud Volumes ONTAP 9.18.1 GA

Vous pouvez désormais utiliser la NetApp Console pour déployer et gérer la version à disponibilité générale (GA) de Cloud Volumes ONTAP 9.18.1 sur AWS, Azure et Google Cloud.

link:https://docs.netapp.com/us-en/cloud-volumes-ontap-9181-relnotes/["En savoir plus sur cette version de Cloud Volumes ONTAP"^].



[[9-february-2026]]
== 9 février 2026



[[support-for-google-cloud-infrastructure-manager]]
=== Prise en charge de Google Cloud Infrastructure Manager

Cloud Volumes ONTAP 9.16.1 et versions ultérieures prennent désormais en charge  https://docs.cloud.google.com/infrastructure-manager/docs/overview["Google Cloud Infrastructure Manager"^] (IM) au lieu de  https://docs.cloud.google.com/deployment-manager/docs/deprecations["Cloud Deployment Manager"^] (DM) pour les nouveaux déploiements sur Google Cloud. Google va déprécier Deployment Manager en tant que service d'infrastructure dans un avenir proche au profit d'Infrastructure Manager, plus avancé.

À compter du 9 février 2026, Cloud Volumes ONTAP utilise Infrastructure Manager pour les déploiements nouveaux et existants. Ce tableau explique quelques flux de travail pour vous :

[cols="1,4,1,1,1,1"]
|===
| Scénario | Action | Nouvelles API pour l'agent | Nouvelles autorisations pour l'agent | Nouvelles API Google Cloud pour Cloud Volumes ONTAP | Ressources documentaires 


| Agent existant et déploiement Cloud Volumes ONTAP existant | Ajoutez de nouvelles API et autorisations à l'agent existant et convertissez le système Cloud Volumes ONTAP existant.  a| 
* API Cloud Infrastructure Manager
* API Cloud Quotas

 a| 
Toutes les autorisations listées pour la version Console sur :

* https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#08-december-2025["8 décembre 2025"^]
* https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#09-february-2026["09 février 2026"^]

| \https://www.googleapis.com/upload/storage/v1 \https://config.googleapis.com/v1  a| 
https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-gcp-dm-im-conv.html["Configurer les déploiements Cloud Volumes ONTAP existants pour Google Cloud Infrastructure Manager"^]



| Agent existant et nouveau déploiement Cloud Volumes ONTAP | Ajoutez de nouvelles API et autorisations à l'agent existant et déployez un nouveau système Cloud Volumes ONTAP.  a| 
* API Cloud Infrastructure Manager
* API Cloud Quotas

 a| 
Toutes les autorisations listées pour la version Console sur :

* https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#08-december-2025["8 décembre 2025"^]
* https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#09-february-2026["09 février 2026"^]

| Toutes les étapes pour un nouveau déploiement  a| 
https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-getting-started-gcp.html["Premiers pas avec Cloud Volumes ONTAP dans Google Cloud"^]



| Nouvel agent et nouveau déploiement | Créez un nouvel agent et déployez un nouveau système Cloud Volumes ONTAP. |  |  |   a| 
* https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html["Créer un agent de Console à partir de Google Cloud"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-getting-started-gcp.html["Premiers pas avec Cloud Volumes ONTAP dans Google Cloud"^]


|===
Déployez Cloud Volumes ONTAP pour qu'il utilise automatiquement Infrastructure Manager ou basculez vos déploiements existants dans Deployment Manager vers Infrastructure Manager en exécutant un outil de conversion. La conversion est un processus unique, après quoi vos systèmes commenceront à utiliser Infrastructure Manager. Reportez-vous à  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-gcp-dm-im-conv.html["Configurer les déploiements Cloud Volumes ONTAP existants pour Google Cloud Infrastructure Manager"^] pour obtenir des instructions sur l'exécution de l'outil de conversion.

Les systèmes Cloud Volumes ONTAP qui utilisent Infrastructure Manager utilisent des buckets Google Cloud Storage pour stocker les données et les enregistrements dans la zone du premier déploiement afin de conserver les enregistrements de déploiement, qui sont réutilisés pour les déploiements suivants. Des frais supplémentaires peuvent s’appliquer pour ces buckets, mais ne modifiez ni ne supprimez les buckets ou leur contenu :

* `gs://netapp-cvo-infrastructure-manager-<project id>` : pour les versions ONTAP et les modèles Terraform SVM utilisés pour les nouveaux déploiements Cloud Volumes ONTAP. À l'intérieur, le compartiment  `dm-to-im-convert` contient les fichiers Terraform de Cloud Volumes ONTAP.
* `<gcp project number>-<region>-blueprint-config`: pour stocker les artefacts Google Cloud Terraform.


.Liens connexes
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-getting-started-gcp.html["Premiers pas avec Cloud Volumes ONTAP dans Google Cloud"^]
* https://docs.netapp.com/us-en/console-setup-admin/whats-new.html#console-agent-4-2-0["NetApp Console Agent 4.2.0 Notes de version"^]
* https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#gcp-permissions-change-log["Autorisations requises pour Google Cloud Infrastructure Manager"^]




[[12-january-2026]]
== 12 janvier 2026



[[preferred-billing-option-for-cloud-volumes-ontap]]
=== Option de facturation privilégiée pour Cloud Volumes ONTAP

Vous pouvez désormais sélectionner l'option de facturation qui vous convient le mieux pour calculer votre consommation et vos dépassements Cloud Volumes ONTAP . Étant donné la disponibilité limitée du modèle de licence Bring Your Own Licenses (BYOL) le 25 juin 2025, NetApp a ajouté des méthodes de facturation préférées dans la section *Licences et abonnements* de la NetApp Console. Vous pouvez utiliser soit votre abonnement annuel à la plateforme pour la facturation et les dépassements, soit le modèle BYOL existant comme option privilégiée. Cela vous permet de choisir la méthode de facturation qui correspond le mieux à la stratégie financière et aux habitudes d'utilisation de votre organisation.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing-charging.html#billing-preferences-and-overages["Préférences de facturation et dépassements"^].



[[10-december-2025]]
== 10 décembre 2025



[[ability-to-enhance-performance-of-premium-ssd-v2-disks-in-azure]]
=== Possibilité d'améliorer les performances des disques SSD Premium v2 dans Azure

Vous pouvez désormais améliorer les performances des disques gérés Premium SSD v2 dans Azure en modifiant les paramètres IOPS et de débit.  Grâce à cette fonctionnalité, vous pouvez optimiser les performances de stockage de vos systèmes en fonction des exigences de votre charge de travail.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-perf-params-azure.html["Gérer les performances des disques SSD Premium v2 pour Cloud Volumes ONTAP dans Azure"^].



[[overage-charging-for-essentials-licenses-simplified]]
=== La facturation des dépassements pour les licences Essentials a été simplifiée.

Pour les contrats annuels et les offres privées Cloud Volumes ONTAP de la place de marché, le calcul des dépassements de capacité pour les licences Essentials est désormais aligné sur les packages Bring-Your-Own License (BYOL). Auparavant, les dépassements étaient facturés aux tarifs horaires de la place de marché pour le package Essentials exact. Désormais, si votre contrat annuel de la place de marché inclut plusieurs packages Essentials, la NetApp Console facture les dépassements d’un package Essentials en fonction de la capacité disponible d’un package Essentials plus onéreux dans votre abonnement. Cela simplifie le calcul des dépassements pour les packages Essentials et garantit une transition fluide du modèle BYOL vers un modèle basé sur l’abonnement.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing-charging.html#how-overages-are-charged-for-essentials-licenses["Comment les dépassements sont facturés pour les licences Essentials"^]



[[support-for-azure-edsv6-sizes-series]]
=== Prise en charge des séries de tailles Azure Edsv6

À partir de Cloud Volumes ONTAP 9.17.1, vous pouvez déployer des machines virtuelles Azure Edsv6 via la NetApp Console pour les nouvelles instances Cloud Volumes ONTAP .  Cloud Volumes ONTAP 9.17.1 et versions ultérieures ne prendront en charge que les machines virtuelles de génération 2 pour les nouveaux déploiements.  Ces machines de deuxième génération sont compatibles avec les technologies les plus récentes, telles que l'interface de microprogramme extensible unifiée (UEFI), les systèmes Azure Boost et NVMe.  Ils sont idéaux pour les systèmes et applications gourmands en mémoire qui nécessitent un stockage local rapide, tels que les serveurs de bases de données et les moteurs d'analyse.

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html#supported-configurations-by-license["Configurations prises en charge pour Cloud Volumes ONTAP dans Azure"^]



[[10-november-2025]]
== 10 novembre 2025



[[enhanced-nvme-tcp-support]]
=== Prise en charge NVMe-TCP améliorée

Auparavant, lors du déploiement d'instances Cloud Volumes ONTAP sur NVMe-TCP, il était nécessaire d'obtenir et d'appliquer manuellement les licences NVMe avant le déploiement.  Grâce à cette mise à jour, Cloud Volumes ONTAP installe désormais automatiquement les licences NVMe requises lors du déploiement, simplifiant ainsi le processus d'installation.

Pour les déploiements NVMe-TCP existants dépourvus de licences, Cloud Volumes ONTAP applique automatiquement les licences.  Vous devez redémarrer le système pour que les licences soient prises en compte.

Pour plus d’informations, consultez https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-client-protocols.html#nvme-tcp["Protocoles clients pris en charge pour Cloud Volumes ONTAP: NVMe-TCP"^].



[[17-october-2025]]
== 17 octobre 2025



[[cloud-volumes-ontap-in-azure-now-limited-to-latest-supported-versions]]
=== Cloud Volumes ONTAP dans Azure est désormais limité aux dernières versions prises en charge

Les déploiements et mises à niveau de Cloud Volumes ONTAP dans Azure via la NetApp Console sont désormais limités aux dernières versions prises en charge. Cela garantit la compatibilité avec le matériel de dernière génération pris en charge par Microsoft et fournit les fonctionnalités et améliorations de sécurité les plus récentes. La console vous invitera à effectuer une mise à niveau vers les versions prises en charge.

Pour plus de détails, reportez-vous à :

* Déploiement: https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-versions.html#azure["Versions ONTAP prises en charge pour les déploiements Cloud Volumes ONTAP"^]
* Mise à niveau: https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html#supported-upgrade-paths["Chemins de mise à niveau pris en charge pour Azure"^]




[[06-october-2025]]
== 06 octobre 2025



[[bluexp-is-now-netapp-console]]
=== BlueXP est désormais NetApp Console

La NetApp Console, construite sur la base BlueXP améliorée et restructurée, fournit une gestion centralisée du stockage NetApp et des NetApp Data Services dans les environnements sur site et cloud à l'échelle de l'entreprise, offrant des informations en temps réel, des flux de travail plus rapides et une administration simplifiée, hautement sécurisée et conforme.

Pour plus de détails sur ce qui a changé, consultez le https://docs.netapp.com/us-en/bluexp-relnotes/index.html["Notes de version de la NetApp Console"^] .



[[simplified-cloud-volumes-ontap-deployment-in-aws]]
=== Déploiement simplifié de Cloud Volumes ONTAP dans AWS

Vous pouvez désormais déployer Cloud Volumes ONTAP dans AWS à l'aide d'une méthode de déploiement rapide pour les configurations à nœud unique et à haute disponibilité (HA).  Ce processus simplifié réduit le nombre d’étapes par rapport à la méthode avancée, définit automatiquement les valeurs par défaut sur une seule page et minimise la navigation, rendant le déploiement plus rapide et plus facile.

Pour plus d'informations, consultez  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-quick-deploy-aws.html["Déployer Cloud Volumes ONTAP dans AWS à l'aide d'un déploiement rapide"^].



[[04-september-2025]]
== 4 septembre 2025



[[cloud-volumes-ontap-9-17-1-rc]]
=== Cloud Volumes ONTAP 9.17.1 RC

Vous pouvez désormais utiliser BlueXP pour déployer et gérer la version Release Candidate 1 de Cloud Volumes ONTAP 9.17.1 dans Azure et Google Cloud. Cependant, cette version n'est pas disponible pour le déploiement et la mise à niveau dans AWS.

link:https://docs.netapp.com/us-en/cloud-volumes-ontap-9171-relnotes/["En savoir plus sur cette version de Cloud Volumes ONTAP"^].



[[11-august-2025]]
== 11 août 2025



[[end-of-availability-of-optimized-licenses]]
=== Fin de disponibilité des licences optimisées

À compter du 11 août 2025, la licence Cloud Volumes ONTAP Optimized sera obsolète et ne sera plus disponible à l’achat ou au renouvellement sur les places de marché Azure et Google Cloud pour les abonnements à la carte (PAYGO). Si vous disposez d'un contrat annuel existant avec une licence optimisée, vous pouvez continuer à utiliser la licence jusqu'à la fin de votre contrat. Lorsque votre licence optimisée expire, vous pouvez opter pour les licences Cloud Volumes ONTAP Essentials ou Professional dans BlueXP.

Cependant, la possibilité d’ajouter ou de renouveler des licences optimisées sera disponible via les API.

Pour plus d'informations sur les formules de licence, veuillez consulter https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html["Licences pour Cloud Volumes ONTAP"^].

Pour plus d'informations sur le passage à une autre méthode de facturation, veuillez consulter https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-capacity-licenses.html["Gérer les licences basées sur la capacité"^].



[[14-july-2025]]
== 14 juillet 2025



[[support-for-transparent-proxy]]
=== Prise en charge du proxy transparent

BlueXP prend désormais en charge les serveurs proxy transparents en plus des connexions proxy explicites existantes.  Lors de la création ou de la modification du connecteur BlueXP , vous pouvez configurer un serveur proxy transparent pour gérer en toute sécurité le trafic réseau vers et depuis Cloud Volumes ONTAP.

Pour plus d'informations sur l'utilisation des serveurs proxy dans Cloud Volumes ONTAP, reportez-vous à :

* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-aws.html#network-configurations-to-support-connector-proxy-servers["Configurations réseau pour prendre en charge le proxy Connector dans AWS"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/azure/reference-networking-azure.html#network-configurations-to-support-connector["Configurations réseau pour prendre en charge le proxy Connector dans Azure"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#network-configurations-to-support-connector-proxy["Configurations réseau pour prendre en charge le proxy Connector dans Google Cloud"^]




[[new-vm-type-supported-for-cloud-volumes-ontap-in-azure]]
=== Nouveau type de machine virtuelle pris en charge pour Cloud Volumes ONTAP dans Azure

À partir de Cloud Volumes ONTAP 9.13.1, L8s_v3 est pris en charge en tant que type de machine virtuelle dans les zones de disponibilité uniques et multiples Azure, pour les déploiements de paires haute disponibilité (HA) nouveaux et existants.

Pour plus d'informations, reportez-vous à https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html["Configurations prises en charge dans Azure"^] .



[[25-june-2025]]
== 25 juin 2025



[[restricted-availability-of-byol-licensing-for-cloud-volumes-ontap]]
=== Disponibilité restreinte des licences BYOL pour Cloud Volumes ONTAP

À compter du 25 juin 2025, NetApp a restreint le modèle de licence Bring-Your-Own License (BYOL) pour Cloud Volumes ONTAP. La restriction s'applique à tous les clients et à tous les déploiements de Cloud Volumes ONTAP sur AWS, Azure et Google Cloud. Les seules exemptions concernent les clients du secteur public américain et les déploiements dans la région de Chine.

Le support et les services NetApp seront maintenus jusqu'à l'expiration de votre contrat BYOL, mais vos licences expirées ne seront ni renouvelées ni prolongées. À l'expiration de vos licences BYOL, vous devrez les remplacer par des licences basées sur la capacité, achetées via vos abonnements à la place de marché cloud. Un modèle de licence basé sur la capacité, via les places de marché hyperscale, simplifie l'expérience de licence et offre de meilleurs avantages commerciaux. Contactez votre équipe de comptes NetApp ou vos représentants du service client pour discuter des options de conversion.

Pour plus d'informations, reportez-vous à ce communiqué client :  https://mysupport.netapp.com/info/communications/CPC-00661.html["CPC-00661 : Modifications de la politique BYOL de Cloud Volumes ONTAP"^] .



[[29-may-2025]]
== 29 mai 2025



[[private-mode-deployments-enabled-for-cloud-volumes-ontap-9-15-1]]
=== Déploiements en mode privé activés pour Cloud Volumes ONTAP 9.15.1

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.15.1 en mode privé dans AWS, Azure et Google Cloud.  Le mode privé est activé pour les déploiements à nœud unique et à haute disponibilité (HA) de Cloud Volumes ONTAP 9.15.1.

Pour plus d'informations sur les déploiements en mode privé, veuillez consulter https://docs.netapp.com/us-en/console-setup-admin/concept-modes.html#restricted-mode["En savoir plus sur les modes de déploiement de BlueXP"^].



[[12-may-2025]]
== 12 mai 2025



[[discovery-of-deployments-made-through-the-azure-marketplace-in-bluexp]]
=== Découverte des déploiements réalisés via la marketplace Azure dans BlueXP

BlueXP a désormais la capacité de découvrir les systèmes Cloud Volumes ONTAP déployés directement via la place de marché Azure.  Cela signifie que vous pouvez désormais ajouter et gérer ces systèmes en tant qu'environnements de travail dans BlueXP, comme tout autre système Cloud Volumes ONTAP .

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-deploy-cvo-azure-mktplc.html["Déployer Cloud Volumes ONTAP depuis la place de marché Azure"^]



[[16-april-2025]]
== 16 avril 2025



[[new-regions-supported-in-azure]]
=== Nouvelles régions prises en charge dans Azure

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures dans des zones de disponibilité uniques et multiples dans Azure dans les régions suivantes. Cela inclut la prise en charge des déploiements à nœud unique et à haute disponibilité (HA).

* Espagne centrale
* Mexique central


Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[14-april-2025]]
== 14 avril 2025



[[storage-vm-creation-automated-through-the-apis-in-google-cloud]]
=== Création de machines virtuelles de stockage automatisée via les API de Google Cloud

Vous pouvez désormais utiliser les API BlueXP pour automatiser la création de machines virtuelles de stockage dans Google Cloud.  Vous avez utilisé cette fonctionnalité dans les configurations haute disponibilité (HA) de Cloud Volumes ONTAP et vous pouvez désormais également l’utiliser dans les déploiements à nœud unique.  En utilisant les API BlueXP , vous pouvez facilement créer, renommer et supprimer des machines virtuelles de stockage de données supplémentaires dans votre environnement Google Cloud, sans avoir à configurer manuellement les interfaces réseau, les LIF et les LIF de gestion requis.  Cette automatisation simplifie le processus de gestion des machines virtuelles de stockage.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-managing-svms-gcp.html["Gérer les machines virtuelles de stockage de données pour Cloud Volumes ONTAP dans Google Cloud"^]



[[03-april-2025]]
== 03 avril 2025



[[support-for-china-regions-for-cloud-volumes-ontap-9-13-1-in-aws]]
=== Prise en charge des régions chinoises pour Cloud Volumes ONTAP 9.13.1 dans AWS

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.13.1 dans AWS dans les régions chinoises. Cela inclut la prise en charge des déploiements à nœud unique et à haute disponibilité (HA). Seules les licences achetées directement auprès de NetApp sont prises en charge.

Pour connaître la disponibilité régionale, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Cartes des régions mondiales pour Cloud Volumes ONTAP"^] .



[[28-march-2025]]
== 28 mars 2025



[[private-mode-deployments-enabled-for-cloud-volumes-ontap-9-14-1]]
=== Déploiements en mode privé activés pour Cloud Volumes ONTAP 9.14.1

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.14.1 en mode privé dans AWS, Azure et Google Cloud.  Le mode privé est activé pour les déploiements à nœud unique et à haute disponibilité (HA) de Cloud Volumes ONTAP 9.14.1.

Pour plus d'informations sur les déploiements en mode privé, veuillez consulter https://docs.netapp.com/us-en/console-setup-admin/concept-modes.html#restricted-mode["En savoir plus sur les modes de déploiement de BlueXP"^].



[[12-march-2025]]
== 12 mars 2025



[[new-regions-supported-for-multiple-availability-zone-deployments-in-azure]]
=== Nouvelles régions prises en charge pour les déploiements de zones de disponibilité multiples dans Azure

Les régions suivantes prennent désormais en charge les déploiements de zones de disponibilité multiples HA dans Azure pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures :

* Centre des États-Unis
* Gouverneur des États-Unis de Virginie (Région gouvernementale des États-Unis - Virginie)


Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[10-march-2025]]
== 10 mars 2025



[[storage-vm-creation-automated-through-the-apis-in-azure]]
=== Création de machines virtuelles de stockage automatisée via les API dans Azure

Vous pouvez désormais utiliser les API BlueXP pour créer, renommer et supprimer des machines virtuelles de stockage de données supplémentaires pour Cloud Volumes ONTAP dans Azure.  L'utilisation des API automatise le processus de création de machines virtuelles de stockage, y compris la configuration des interfaces réseau requises, des LIF et d'une LIF de gestion, si vous devez utiliser une machine virtuelle de stockage à des fins de gestion.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-managing-svms-azure.html["Gérer les machines virtuelles de stockage de données pour Cloud Volumes ONTAP dans Azure"^]



[[06-march-2025]]
== 06 mars 2025



[[cloud-volumes-ontap-9-16-1-ga]]
=== Cloud Volumes ONTAP 9.16.1 GA

Vous pouvez désormais utiliser BlueXP pour déployer et gérer la version de disponibilité générale de Cloud Volumes ONTAP 9.16.1 dans Azure et Google Cloud. Cependant, cette version n'est pas disponible pour le déploiement et la mise à niveau dans AWS.

link:https://docs.netapp.com/us-en/cloud-volumes-ontap-9161-relnotes/["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[03-march-2025]]
== 03 mars 2025



[[support-for-new-zealand-north-region-in-azure]]
=== Prise en charge de la région Nouvelle-Zélande Nord dans Azure

La région Nouvelle-Zélande Nord est désormais prise en charge dans Azure pour les configurations à nœud unique et à haute disponibilité (HA) de Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.  Notez que le type d’instance Lsv3 n’est pas pris en charge dans cette région.

Pour une liste de toutes les régions prises en charge, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[18-february-2025]]
== 18 février 2025



[[introducing-azure-marketplace-direct-deployment]]
=== Présentation du déploiement direct sur la place de marché Azure

Vous pouvez désormais profiter du déploiement direct de la place de marché Azure pour déployer facilement et rapidement Cloud Volumes ONTAP directement depuis la place de marché Azure.  Grâce à cette méthode simplifiée, vous pouvez explorer les principales fonctionnalités et capacités de Cloud Volumes ONTAP dans votre environnement sans avoir à configurer le connecteur BlueXP ou à répondre à d'autres critères d'intégration requis pour le déploiement de Cloud Volumes ONTAP via BlueXP.

* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-azure-mktplace-direct.html["Découvrez les options de déploiement de Cloud Volumes ONTAP dans Azure"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-deploy-cvo-azure-mktplc.html["Déployer Cloud Volumes ONTAP depuis la place de marché Azure"^]




[[10-february-2025]]
== 10 février 2025



[[user-authentication-enabled-for-accessing-system-manager-from-bluexp]]
=== Authentification utilisateur activée pour accéder au Gestionnaire système depuis BlueXP

En tant qu'administrateur BlueXP , vous pouvez désormais activer l'authentification pour les utilisateurs ONTAP accédant à ONTAP System Manager depuis BlueXP.  Vous pouvez activer cette option en modifiant les paramètres du connecteur BlueXP .  Cette option est disponible pour les modes standard et privé.

link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-administer-advanced-view.html["Administrer Cloud Volumes ONTAP à l'aide de System Manager"^].



[[bluexp-advanced-view-renamed-to-system-manager]]
=== BlueXP Advanced View renommé en Gestionnaire système

L'option de gestion avancée de Cloud Volumes ONTAP depuis BlueXP via ONTAP System Manager a été renommée de *Vue avancée* à *Gestionnaire système*.

link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-administer-advanced-view.html["Administrer Cloud Volumes ONTAP à l'aide de System Manager"^].



[[introducing-a-simpler-way-to-manage-licenses-with-the-bluexp-digital-wallet]]
=== Présentation d'un moyen plus simple de gérer les licences avec le BlueXP digital wallet

Vous pouvez désormais bénéficier d'une gestion simplifiée des licences Cloud Volumes ONTAP en utilisant des points de navigation améliorés au sein du BlueXP digital wallet:

* Accédez facilement aux informations de votre licence Cloud Volumes ONTAP via les onglets *Administration > Licenses and subscriptions > Présentation/Licences directes*.
* Cliquez sur *Afficher* dans le panneau Cloud Volume ONTAP dans l'onglet *Présentation* pour obtenir une compréhension complète de vos licences basées sur la capacité.  Cette vue avancée offre un aperçu détaillé de vos licences et abonnements.
* Si vous préférez l'interface précédente, vous pouvez cliquer sur le bouton *Passer à la vue héritée* pour afficher les détails de la licence par type et modifier les méthodes de facturation de vos licences.


link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-capacity-licenses.html["Gérer les licences basées sur la capacité"^].



[[09-december-2024]]
== 09 décembre 2024



[[list-of-supported-vms-updated-for-azure-to-align-with-the-best-practices]]
=== Liste des machines virtuelles prises en charge mise à jour pour Azure afin de s'aligner sur les meilleures pratiques

Les familles de machines DS_v2 et Es_v3 ne sont plus disponibles pour la sélection sur BlueXP lors du déploiement de nouvelles instances de Cloud Volumes ONTAP dans Azure. Ces familles seront conservées et soutenues uniquement dans les systèmes plus anciens et existants. Les nouveaux déploiements de Cloud Volumes ONTAP sont pris en charge dans Azure uniquement à partir de la version 9.12.1. Nous vous recommandons de passer à Es_v4 ou à toute autre série compatible avec Cloud Volumes ONTAP 9.12.1 et versions ultérieures. Les machines des séries DS_v2 et Es_v3 seront toutefois disponibles pour les nouveaux déploiements effectués via l'API.

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html["Configurations prises en charge dans Azure"^]



[[11-november-2024]]
== 11 novembre 2024



[[end-of-availability-for-node-based-licenses]]
=== Fin de disponibilité des licences basées sur des nœuds

NetApp a planifié la fin de la disponibilité (EOA) et la fin du support (EOS) des licences basées sur les nœuds Cloud Volumes ONTAP .  À compter du 11 novembre 2024, la disponibilité limitée des licences basées sur des nœuds a pris fin. Le support des licences basées sur les nœuds prend fin le 31 décembre 2024.  Après l'EOA de vos licences basées sur les nœuds, vous devez passer à des licences basées sur la capacité en utilisant l'outil de conversion de licence BlueXP .

Pour les engagements annuels ou à plus long terme, NetApp vous recommande de contacter votre représentant NetApp avant la date EOA ou la date d'expiration de la licence pour vous assurer que les conditions préalables à la transition sont en place.  Si vous ne disposez pas d'un contrat à long terme pour un nœud Cloud Volumes ONTAP et que vous exécutez votre système avec un abonnement à la demande (PAYGO), il est important de planifier votre conversion avant la date EOS.  Pour les contrats à long terme et les abonnements PAYGO, vous pouvez utiliser l'outil de conversion de licence BlueXP pour une conversion transparente.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#end-of-availability-of-node-based-licenses["Fin de disponibilité des licences basées sur des nœuds"^] https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-convert-node-capacity.html["Convertir une licence basée sur un nœud Cloud Volumes ONTAP en une licence basée sur la capacité"^]



[[removal-of-node-based-deployments-from-bluexp]]
=== Suppression des déploiements basés sur des nœuds de BlueXP

L'option permettant de déployer des systèmes Cloud Volumes ONTAP à l'aide de licences basées sur des nœuds est obsolète sur BlueXP.  À l’exception de quelques cas particuliers, vous ne pouvez pas utiliser de licences basées sur des nœuds pour les déploiements Cloud Volumes ONTAP pour n’importe quel fournisseur de cloud.

NetApp reconnaît les exigences de licence uniques suivantes conformément aux obligations contractuelles et aux besoins opérationnels, et continuera à prendre en charge les licences basées sur les nœuds dans ces situations :

* Clients du secteur public américain
* Déploiements en mode privé
* Déploiements de Cloud Volumes ONTAP dans AWS en Chine
* Si vous possédez un package Bring-Your-Own License (BYOL) valide et non expiré par nœud


https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#end-of-availability-of-node-based-licenses["Fin de disponibilité des licences basées sur des nœuds"^]



[[addition-of-a-cold-tier-for-cloud-volumes-ontap-data-on-azure-blob-storage]]
=== Ajout d'un niveau froid pour les données Cloud Volumes ONTAP sur le stockage Azure Blob

BlueXP vous permet désormais de sélectionner un niveau froid pour stocker les données de niveau de capacité inactives sur le stockage Azure Blob.  L'ajout du niveau froid aux niveaux chaud et froid existants vous offre une option de stockage plus abordable et une meilleure rentabilité.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-data-tiering.html#data-tiering-in-azure["Hiérarchisation des données dans Azure"^]



[[option-to-restrict-public-access-to-storage-account-for-azure]]
=== Option permettant de restreindre l'accès public au compte de stockage pour Azure

Vous avez désormais la possibilité de restreindre l’accès public à votre compte de stockage pour les systèmes Cloud Volumes ONTAP dans Azure.  En désactivant l'accès, vous pouvez sécuriser votre adresse IP privée contre toute exposition même au sein du même réseau virtuel, s'il est nécessaire de se conformer aux politiques de sécurité de votre organisation.  Cette option désactive également la hiérarchisation des données pour vos systèmes Cloud Volumes ONTAP et s'applique aux paires à nœud unique et à haute disponibilité.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-azure.html#security-group-rules["Règles du groupe de sécurité"^].



[[worm-enablement-after-deploying-cloud-volumes-ontap]]
=== Activation de WORM après le déploiement de Cloud Volumes ONTAP

Vous avez désormais la possibilité d'activer le stockage WORM (Write Once, Read Many) sur un système Cloud Volumes ONTAP existant à l'aide de BlueXP.  Cette fonctionnalité vous offre la flexibilité d'activer WORM sur un environnement de travail, même si WORM n'a pas été activé sur celui-ci lors de sa création.  Une fois activé, vous ne pouvez pas désactiver WORM.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-worm.html#enabling-worm-on-a-cloud-volumes-ontap-working-environment["Activation de WORM sur un environnement de travail Cloud Volumes ONTAP"^]



[[25-october-2024]]
== 25 octobre 2024



[[list-of-supported-vms-updated-for-google-cloud-to-align-with-the-best-practices]]
=== Liste des machines virtuelles prises en charge mise à jour pour Google Cloud afin de s'aligner sur les meilleures pratiques

Les machines de la série n1 ne sont plus disponibles pour la sélection sur BlueXP lors du déploiement de nouvelles instances de Cloud Volumes ONTAP dans Google Cloud. Les machines de la série n1 seront conservées et prises en charge uniquement dans les systèmes plus anciens et existants. Les nouveaux déploiements de Cloud Volumes ONTAP sont pris en charge dans Google Cloud uniquement à partir de la version 9.8.  Nous vous recommandons de passer aux types de machines de la série n2 compatibles avec Cloud Volumes ONTAP 9.8 et versions ultérieures. Les machines de la série n1 seront toutefois disponibles pour de nouveaux déploiements effectués via l'API.

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-gcp.html["Configurations prises en charge dans Google Cloud"^] .



[[local-zones-support-for-amazon-web-services-in-private-mode]]
=== Prise en charge des zones locales pour Amazon Web Services en mode privé

BlueXP prend désormais en charge les zones locales AWS pour les déploiements haute disponibilité (HA) Cloud Volumes ONTAP en mode privé.  Le support qui était auparavant limité au mode standard uniquement a désormais été étendu pour inclure le mode privé.


NOTE: Les zones locales AWS ne sont pas prises en charge lors de l'utilisation de BlueXP en mode restreint.

Pour plus d'informations sur les zones locales AWS avec des déploiements HA, consultez link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-ha.html#aws-local-zones["Zones locales AWS"^].



[[07-october-2024]]
== 07 octobre 2024



[[enhanced-user-experience-in-version-selection-for-upgrade]]
=== Expérience utilisateur améliorée dans la sélection de la version pour la mise à niveau

À partir de cette version, lorsque vous essayez de mettre à niveau Cloud Volumes ONTAP à l'aide de la notification BlueXP , vous recevrez des conseils sur les versions par défaut, les plus récentes et compatibles à utiliser.  Vous pouvez également désormais sélectionner le dernier correctif ou la version majeure compatible avec votre instance Cloud Volumes ONTAP , ou saisir manuellement une version à mettre à niveau.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html#upgrade-from-console-notifications["Mettre à niveau le logiciel Cloud Volumes ONTAP"]



[[09-september-2024]]
== 09 septembre 2024



[[worm-and-arp-functionalities-are-no-longer-chargeable]]
=== Les fonctionnalités WORM et ARP ne sont plus payantes

Les fonctionnalités intégrées de protection des données et de sécurité de WORM (Write Once Read Many) et ARP (Autonomous Ransomware Protection) seront proposées avec les licences Cloud Volumes ONTAP sans frais supplémentaires.  Le nouveau modèle de tarification s'applique aux abonnements BYOL et PAYGO/marketplace nouveaux et existants d'AWS, Azure et Google Cloud.  Les licences basées sur la capacité et sur les nœuds contiendront ARP et WORM pour toutes les configurations, y compris les paires à nœud unique et à haute disponibilité (HA), sans frais supplémentaires.

La tarification simplifiée vous apporte les avantages suivants :

* Les comptes qui incluent actuellement WORM et ARP n'entraîneront plus de frais pour ces fonctionnalités.  À l’avenir, votre facturation ne comportera que des frais d’utilisation de la capacité, comme c’était le cas avant ce changement.  WORM et ARP ne seront plus inclus dans vos futures factures.
* Si vos comptes actuels n'incluent pas ces fonctionnalités, vous pouvez désormais opter pour WORM et ARP sans frais supplémentaires.
* Toutes les offres Cloud Volumes ONTAP pour tous les nouveaux comptes excluront les frais pour WORM et ARP.


En savoir plus sur ces fonctionnalités :

* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-protecting-ransomware.html["Activer les solutions de protection contre les ransomwares NetApp pour Cloud Volumes ONTAP"]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-worm.html["Stockage de l'WORM"]




[[23-august-2024]]
== 23 août 2024



[[canada-west-region-now-supported-in-aws]]
=== La région Canada Ouest est désormais prise en charge dans AWS

La région Canada Ouest est désormais prise en charge dans AWS pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.

Pour une liste de toutes les régions, voir le https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous AWS"^] .



[[22-august-2024]]
== 22 août 2024



[[cloud-volumes-ontap-9-15-1-ga]]
=== Cloud Volumes ONTAP 9.15.1 GA

BlueXP peut désormais déployer et gérer la version de disponibilité générale de Cloud Volumes ONTAP 9.15.1 dans AWS, Azure et Google Cloud.

Cette version inclut les fonctionnalités et améliorations suivantes pour Azure. Poursuivez votre lecture pour en savoir plus sur les configurations spécifiques prenant en charge ces nouvelles fonctionnalités.



[[support-for-premium-ssd-v2-managed-disks-in-azure]]
=== Prise en charge des disques gérés Premium SSD v2 dans Azure

Les disques managés Premium SSD v2 sont désormais pris en charge dans les systèmes Cloud Volumes ONTAP sur Azure. Ces disques Premium offrent des performances supérieures avec une latence réduite à moindre coût, aussi bien pour les nœuds uniques que pour les paires haute disponibilité (HA), par rapport aux disques managés Premium SSD. Comme les autres disques managés, la taille maximale d’un disque est de 32 Tio. Les performances (capacité, débit et IOPS) d’un disque managé Premium SSD v2 peuvent être configurées, ce qui permet d’optimiser les coûts des charges de travail tout en répondant à l’évolution des besoins en performances.



[[deploy-ha-pairs-in-single-availability-zones-in-azure]]
=== Déployez des paires haute disponibilité dans des zones de disponibilité uniques dans Azure

À partir de Cloud Volumes ONTAP 9.15.1, vous pouvez déployer des instances de machines virtuelles (VM) en mode haute disponibilité (HA) dans une seule zone de disponibilité (AZ) dans Azure. Contrairement aux déploiements non zonés précédents, Cloud Volumes ONTAP 9.15.1 utilise Microsoft Virtual Machines Scale Sets en mode d'orchestration flexible pour déployer toutes les ressources, y compris des domaines de panne distincts au sein de la même AZ, garantissant une disponibilité optimale. Ce mode de déploiement utilise par défaut des disques managés SSD v2 Premium lorsque les conditions suivantes sont remplies :

* La version de Cloud Volumes ONTAP est 9.15.1 ou ultérieure.
* La région et la zone sélectionnées prennent en charge les disques SSD Premium v2 gérés. Pour plus d’informations sur les régions prises en charge, consultez  https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/["Site Web Microsoft Azure : produits disponibles par région"^]. Pour savoir comment les ajouter, consultez  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-deploying-otc-azure.html#launching-a-cloud-volumes-ontap-ha-pair-in-azure["Lancement d'une paire haute disponibilité Cloud Volumes ONTAP dans Azure"^].
* L'abonnement est enregistré pour la fonctionnalité  `Microsoft.Compute/VMOrchestratorZonalMultiFD` Microsoft. https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-saz-feature.html["Découvrez comment activer VMOrchestratorZonalMultiFD pour une seule zone de disponibilité"^].


Si l'un de ces critères n'est pas rempli, le mode de déploiement non zonal précédent pour le stockage localement redondant (LRS) devient effectif.



[[support-for-virtual-machines-scale-sets-to-unify-all-azure-ha-types]]
=== Prise en charge des groupes de machines virtuelles identiques pour unifier tous les types de haute disponibilité Azure

Cloud Volumes ONTAP 9.15.1 exploite les groupes de machines virtuelles identiques en mode d'orchestration flexible sur Azure pour déployer des instances de machines virtuelles (VM) dans des zones de disponibilité uniques pour des paires haute disponibilité (HA). Il couvre toutes les variantes du mode HA, page blob, LRS, stockage redondant par zone (ZRS) ou multizone, et LRS zonal (zone de disponibilité unique).

* https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/["Documentation Microsoft Azure : documentation sur les groupes de machines virtuelles identiques"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-ha-azure.html["Découvrez les paires haute disponibilité dans Azure"^].




[[support-for-flexcache-write-back]]
=== Prise en charge de la réécriture avec FlexCache

À partir de Cloud Volumes ONTAP 9.15.1, la fonction d'écriture différée FlexCache est prise en charge comme mode de fonctionnement alternatif pour l'écriture dans un cache.

Pour plus d'informations sur cette fonctionnalité, consultez la documentation ONTAP https://docs.netapp.com/us-en/ontap/flexcache-writeback/flexcache-write-back-overview.html["FlexCache aperçu de la fonction de réécriture"^].

Pour plus d'informations sur la façon dont BlueXP gère les volumes FlexCache, reportez-vous au https://docs.netapp.com/us-en/console-volume-caching/index.html["Documents de mise en cache de volumes BlueXP"^].



[[08-august-2024]]
== 08 août 2024



[[edge-cache-licensing-packages-deprecated]]
=== Les packages de licences Edge Cache sont obsolètes

Les packages de licences basés sur la capacité Edge Cache ne seront plus disponibles pour les futurs déploiements de Cloud Volumes ONTAP.  Cependant, vous pouvez utiliser l'API pour bénéficier de cette fonctionnalité.



[[minimum-version-support-for-flash-cache-in-azure]]
=== Prise en charge minimale des versions pour Flash Cache dans Azure

La version minimale de Cloud Volumes ONTAP requise pour la configuration de Flash Cache dans Azure est 9.13.1 GA.  Vous ne pouvez utiliser ONTAP 9.13.1 GA et les versions ultérieures pour déployer Flash Cache sur les systèmes Cloud Volumes ONTAP dans Azure.

Pour connaître les configurations prises en charge, veuillez vous référer à https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html#single-node-systems["Configurations prises en charge dans Azure"^].



[[free-trials-for-marketplace-subscriptions-deprecated]]
=== Les essais gratuits pour les abonnements au marché sont obsolètes

La licence d'essai ou d'évaluation gratuite automatique de 30 jours pour les abonnements à la carte sur la place de marché du fournisseur de cloud ne sera plus disponible dans Cloud Volumes ONTAP.  La facturation de tout type d'abonnement marketplace (PAYGO ou contrat annuel) sera activée dès la première utilisation, sans aucune période d'essai gratuite.



[[10-june-2024]]
== 10 juin 2024



[[cloud-volumes-ontap-9-15-0]]
=== Cloud Volumes ONTAP 9.15.0

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.15.0 dans AWS, Azure et Google Cloud.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9150-relnotes/["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[17-may-2024]]
== 17 mai 2024



[[amazon-web-services-local-zones-support]]
=== Prise en charge des zones locales d'Amazon Web Services

La prise en charge des zones locales AWS est désormais disponible pour les déploiements Cloud Volumes ONTAP HA.  Les zones locales AWS sont un déploiement d'infrastructure où le stockage, le calcul, la base de données et d'autres services AWS sélectionnés sont situés à proximité de grandes villes et de zones industrielles.


NOTE: Les zones locales AWS sont prises en charge lors de l'utilisation de BlueXP en mode standard.  À l’heure actuelle, les zones locales AWS ne sont pas prises en charge lors de l’utilisation de BlueXP en mode restreint ou en mode privé.

Pour plus d'informations sur les zones locales AWS avec des déploiements HA, consultez  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-ha.html#aws-local-zones["Zones locales AWS"^].



[[23-april-2024]]
== 23 avril 2024



[[new-regions-supported-for-multiple-availability-zone-deployments-in-azure]]
=== Nouvelles régions prises en charge pour les déploiements de zones de disponibilité multiples dans Azure

Les régions suivantes prennent désormais en charge les déploiements de zones de disponibilité multiples HA dans Azure pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures :

* Allemagne Centre-Ouest
* Pologne centrale
* Ouest des États-Unis 3
* Israël Central
* Italie du Nord
* Canada Central


Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[johannesburg-region-now-supported-in-google-cloud]]
=== La région de Johannesburg est désormais prise en charge dans Google Cloud

La région de Johannesburg(`africa-south1` (région) est désormais pris en charge dans Google Cloud pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.

Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Google Cloud"^] .



[[volume-templates-and-tags-no-longer-supported]]
=== Les modèles et balises de volume ne sont plus pris en charge

Vous ne pouvez plus créer un volume à partir d'un modèle ni modifier les balises d'un volume.  Ces actions étaient associées au service de correction BlueXP , qui n'est plus disponible.



[[08-march-2024]]
== 08 mars 2024



[[amazon-instant-metadata-service-v2-support]]
=== Prise en charge du service de métadonnées instantanées Amazon v2

Dans AWS, Cloud Volumes ONTAP, le médiateur et le connecteur prennent désormais en charge Amazon Instant Metadata Service v2 (IMDSv2) pour toutes les fonctions.  IMDSv2 offre une protection renforcée contre les vulnérabilités.  Seul IMDSv1 était auparavant pris en charge.

Si vos politiques de sécurité l'exigent, vous pouvez configurer vos instances EC2 pour utiliser IMDSv2. Pour obtenir des instructions, consultez  https://docs.netapp.com/us-en/console-setup-admin/task-require-imdsv2.html["Documentation de configuration et d'administration BlueXP pour la gestion des connecteurs existants"^].



[[05-march-2024]]
== 05 mars 2024



[[cloud-volumes-ontap-9-14-1-ga]]
=== Cloud Volumes ONTAP 9.14.1 GA

BlueXP peut désormais déployer et gérer la version de disponibilité générale de Cloud Volumes ONTAP 9.14.1 dans AWS, Azure et Google Cloud.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9141-relnotes/["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[02-february-2024]]
== 02 février 2024



[[support-for-edv5-series-vms-in-azure]]
=== Prise en charge des machines virtuelles de la série Edv5 dans Azure

Cloud Volumes ONTAP prend désormais en charge les machines virtuelles de la série Edv5 suivantes à partir de la version 9.14.1.

* E4ds_v5
* E8ds_v5
* E20s_v5
* E32ds_v5
* E48ds_v5
* E64ds_v5


https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html["Configurations prises en charge dans Azure"^]



[[16-january-2024]]
== 16 janvier 2024



[[patch-releases-in-bluexp]]
=== Versions de correctifs dans BlueXP

Les versions de correctifs sont disponibles dans BlueXP uniquement pour les trois dernières versions de Cloud Volumes ONTAP.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html#patch-releases["Mettre à niveau Cloud Volumes ONTAP"^]



[[08-january-2024]]
== 08 janvier 2024



[[new-vms-for-azure-multiple-availability-zones]]
=== Nouvelles machines virtuelles pour les zones de disponibilité multiples Azure

À partir de Cloud Volumes ONTAP 9.13.1, les types de machines virtuelles suivants prennent en charge plusieurs zones de disponibilité Azure pour les déploiements de paires haute disponibilité nouveaux et existants :

* L16s_v3
* L32s_v3
* L48s_v3
* L64s_v3


https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html["Configurations prises en charge dans Azure"^]



[[06-december-2023]]
== 06 décembre 2023



[[cloud-volumes-ontap-9-14-1-rc1]]
=== Cloud Volumes ONTAP 9.14.1 RC1

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.14.1 dans AWS, Azure et Google Cloud.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9141-relnotes/["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[300-tib-flexvol-volume-max-limit]]
=== Limite maximale de FlexVol volume de 300 TiB

Vous pouvez désormais créer un FlexVol volume jusqu'à la taille maximale de 300 Tio avec System Manager et ONTAP CLI à partir de Cloud Volumes ONTAP 9.12.1 P2 et 9.13.0 P2, et dans BlueXP à partir de Cloud Volumes ONTAP 9.13.1.

* https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-limits-aws.html#file-and-volume-limits["Limites de stockage dans AWS"]
* https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-limits-azure.html#file-and-volume-limits["Limites de stockage dans Azure"]
* https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-limits-gcp.html#logical-storage-limits["Limites de stockage dans Google Cloud"]




[[05-december-2023]]
== 05 décembre 2023

Les changements suivants ont été introduits.



[[new-region-support-in-azure]]
=== Prise en charge de nouvelles régions dans Azure

.Prise en charge d'une région de zone de disponibilité unique
Les régions suivantes prennent désormais en charge les déploiements de zone de disponibilité unique hautement disponibles dans Azure pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures :

* Tel-Aviv
* Milan


.Prise en charge de plusieurs régions de zone de disponibilité
Les régions suivantes prennent désormais en charge les déploiements de zones de disponibilité multiples hautement disponibles dans Azure pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures :

* Inde centrale
* Norvège de l'Est
* Suisse du Nord
* Afrique du Sud Nord
* Émirats arabes unis du Nord


Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[10-november-2023]]
== 10 novembre 2023

Le changement suivant a été introduit avec la version 3.9.35 du connecteur.



[[berlin-region-now-supported-in-google-cloud]]
=== La région de Berlin est désormais prise en charge dans Google Cloud

La région de Berlin est désormais prise en charge dans Google Cloud pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.

Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Google Cloud"^] .



[[08-november-2023]]
== 08 novembre 2023

Le changement suivant a été introduit avec la version 3.9.35 du connecteur.



[[tel-aviv-region-now-supported-in-aws]]
=== La région de Tel Aviv est désormais prise en charge dans AWS

La région de Tel Aviv est désormais prise en charge dans AWS pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.

Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous AWS"^] .



[[01-november-2023]]
== 01 novembre 2023

Le changement suivant a été introduit avec la version 3.9.34 du connecteur.



[[saudi-arabia-region-now-supported-in-google-cloud]]
=== La région de l'Arabie saoudite est désormais prise en charge dans Google Cloud

La région Arabie saoudite est désormais prise en charge dans Google Cloud pour Cloud Volumes ONTAP et Connector pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.

Pour une liste de toutes les régions, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Google Cloud"^] .



[[23-october-2023]]
== 23 octobre 2023

Le changement suivant a été introduit avec la version 3.9.34 du connecteur.



[[new-regions-supported-for-ha-multiple-availability-zone-deployments-in-azure]]
=== Nouvelles régions prises en charge pour les déploiements de zones de disponibilité multiples HA dans Azure

Les régions suivantes dans Azure prennent désormais en charge les déploiements de zones de disponibilité multiples hautement disponibles pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures :

* Australie de l'Est
* Asie de l'Est
* France Centre
* Europe du Nord
* Qatar Central
* Suède centrale
* Europe de l'Ouest
* Ouest des États-Unis 2


Pour obtenir la liste de toutes les régions prenant en charge plusieurs zones de disponibilité, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[06-october-2023]]
== 06 octobre 2023

Le changement suivant a été introduit avec la version 3.9.34 du connecteur.



[[cloud-volumes-ontap-9-14-0]]
=== Cloud Volumes ONTAP 9.14.0

BlueXP peut désormais déployer et gérer la version de disponibilité générale de Cloud Volumes ONTAP 9.14.0 dans AWS, Azure et Google Cloud.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9140-relnotes/["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[10-september-2023]]
== 10 septembre 2023

Le changement suivant a été introduit avec la version 3.9.33 du connecteur.



[[support-for-lsv3-series-vms-in-azure]]
=== Prise en charge des machines virtuelles de la série Lsv3 dans Azure

Les types d’instances L48s_v3 et L64s_v3 sont désormais pris en charge avec Cloud Volumes ONTAP dans Azure pour les déploiements à nœud unique et à paires haute disponibilité avec des disques gérés partagés dans des zones de disponibilité uniques et multiples, à partir de la version 9.13.1.  Ces types d’instances prennent en charge Flash Cache.

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html["Afficher les configurations prises en charge pour Cloud Volumes ONTAP dans Azure"^] https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-limits-azure.html["Afficher les limites de stockage pour Cloud Volumes ONTAP dans Azure"^]



[[30-july-2023]]
== 30 juillet 2023

Les modifications suivantes ont été introduites avec la version 3.9.32 du connecteur.



[[flash-cache-and-high-write-speed-support-in-google-cloud]]
=== Prise en charge du cache Flash et de la vitesse d'écriture élevée dans Google Cloud

Le cache Flash et la vitesse d'écriture élevée peuvent être activés séparément dans Google Cloud pour Cloud Volumes ONTAP 9.13.1 et versions ultérieures.  Une vitesse d'écriture élevée est disponible sur tous les types d'instances pris en charge.  Flash Cache est pris en charge sur les types d’instances suivants :

* n2-standard-16
* n2-standard-32
* n2-standard-48
* n2-standard-64


Vous pouvez utiliser ces fonctionnalités séparément ou ensemble sur des déploiements à nœud unique et à paires haute disponibilité.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-deploying-gcp.html["Lancer Cloud Volumes ONTAP dans Google Cloud"^]



[[usage-reports-enhancements]]
=== Améliorations des rapports d'utilisation

Diverses améliorations des informations affichées dans les rapports d’utilisation sont désormais disponibles.  Voici les améliorations apportées aux rapports d’utilisation :

* L'unité TiB est désormais incluse dans le nom des colonnes.
* Un nouveau champ « nœud(s) » pour les numéros de série est désormais inclus.
* Une nouvelle colonne « Type de charge de travail » est désormais incluse dans le rapport d’utilisation des machines virtuelles de stockage.
* Les noms d’environnement de travail sont désormais inclus dans les rapports d’utilisation des machines virtuelles de stockage et des volumes.
* Le type de volume « fichier » est désormais étiqueté « Principal (lecture/écriture) ».
* Le type de volume « secondaire » est désormais étiqueté « Secondaire (DP) ».


Pour plus d'informations sur les rapports d'utilisation, veuillez consulter https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-capacity-licenses.html#download-usage-reports["Télécharger les rapports d'utilisation"^].



[[26-july-2023]]
== 26 juillet 2023

Les modifications suivantes ont été introduites avec la version 3.9.31 du connecteur.



[[cloud-volumes-ontap-9-13-1-ga]]
=== Cloud Volumes ONTAP 9.13.1 GA

BlueXP peut désormais déployer et gérer la version de disponibilité générale de Cloud Volumes ONTAP 9.13.1 dans AWS, Azure et Google Cloud.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9131-relnotes/["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[02-july-2023]]
== 02 juillet 2023

Les modifications suivantes ont été introduites avec la version 3.9.31 du connecteur.



[[support-for-ha-multiple-availability-zone-deployments-in-azure]]
=== Prise en charge des déploiements de zones de disponibilité multiples HA dans Azure

Le Japon Est et la Corée Centrale dans Azure prennent désormais en charge les déploiements de zones de disponibilité multiples HA pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.

Pour obtenir la liste de toutes les régions prenant en charge plusieurs zones de disponibilité, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Carte des régions mondiales sous Azure"^] .



[[autonomous-ransomware-protection-support]]
=== Prise en charge de la protection autonome contre les ransomwares

La protection autonome contre les ransomwares (ARP) est désormais prise en charge sur Cloud Volumes ONTAP.  La prise en charge ARP est disponible sur Cloud Volumes ONTAP version 9.12.1 et supérieure.

Pour en savoir plus sur ARP avec Cloud Volumes ONTAP, consultez https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-protecting-ransomware.html#autonomous-ransomware-protection["Protection autonome contre les ransomwares"^].



[[26-june-2023]]
== 26 juin 2023

Le changement suivant a été introduit avec la version 3.9.30 du connecteur.



[[cloud-volumes-ontap-9-13-1-rc1]]
=== Cloud Volumes ONTAP 9.13.1 RC1

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.13.1 dans AWS, Azure et Google Cloud.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9131-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[04-june-2023]]
== 4 juin 2023

Le changement suivant a été introduit avec la version 3.9.30 du connecteur.



[[cloud-volumes-ontap-upgrade-version-selector-update]]
=== Mise à jour du sélecteur de version de mise à niveau de Cloud Volumes ONTAP

Grâce à la page Mettre à niveau Cloud Volumes ONTAP , vous pouvez désormais choisir de mettre à niveau vers la dernière version disponible de Cloud Volumes ONTAP ou une version plus ancienne.

Pour en savoir plus sur la mise à niveau de Cloud Volumes ONTAP via BlueXP, reportez-vous à https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-updating-ontap-cloud.html#upgrade-cloud-volumes-ontap["Mettre à niveau Cloud Volumes ONTAP"^] .



[[07-may-2023]]
== 07 mai 2023

Les modifications suivantes ont été introduites avec la version 3.9.29 du connecteur.



[[qatar-region-now-supported-in-google-cloud]]
=== La région du Qatar est désormais prise en charge dans Google Cloud

La région du Qatar est désormais prise en charge dans Google Cloud pour Cloud Volumes ONTAP et Connector pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.



[[sweden-central-region-now-supported-in-azure]]
=== La région Suède Centre est désormais prise en charge dans Azure

La région Suède centrale est désormais prise en charge dans Azure pour Cloud Volumes ONTAP et le connecteur pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.



[[support-for-ha-multiple-availability-zone-deployments-in-azure-australia-east]]
=== Prise en charge des déploiements multizones de haute disponibilité dans Azure Australie Est

La région Australie Est dans Azure prend désormais en charge les déploiements de zones de disponibilité multiples HA pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.



[[charging-usage-breakdown]]
=== Répartition de l'utilisation de la charge

Vous pouvez désormais savoir ce qui vous est facturé lorsque vous êtes abonné à des licences basées sur la capacité.  Les types de rapports d'utilisation suivants sont disponibles en téléchargement à partir du portefeuille numérique dans BlueXP.  Les rapports d'utilisation fournissent des détails sur la capacité de vos abonnements et vous indiquent comment vous êtes facturé pour les ressources de vos abonnements Cloud Volumes ONTAP .  Les rapports téléchargeables peuvent être facilement partagés avec d’autres.

* Utilisation du package Cloud Volumes ONTAP
* Utilisation de haut niveau
* Utilisation des machines virtuelles de stockage
* Utilisation des volumes


Pour plus d'informations, consultez  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-capacity-licenses.html["Gérer les licences basées sur la capacité"^].



[[notification-now-displays-when-accessing-bluexp-without-a-marketplace-subscription]]
=== La notification s'affiche désormais lors de l'accès à BlueXP sans abonnement à la place de marché

Une notification s'affiche désormais chaque fois que vous accédez à Cloud Volumes ONTAP dans BlueXP sans abonnement à la place de marché.  La notification indique qu'« un abonnement au marché pour cet environnement de travail est requis pour être conforme aux conditions générales de Cloud Volumes ONTAP ».



[[new-permissions-added-to-aws-iam-policy-for-ha-mediators]]
=== De nouvelles autorisations ont été ajoutées à la politique AWS IAM pour les médiateurs HA.

Ces nouvelles autorisations AWS ont été ajoutées à la politique IAM pour les médiateurs HA dans les environnements à haute disponibilité (HA) de Cloud Volumes ONTAP :

* sts:Assumer le rôle
* ec2 : Décrire les sous-réseaux




[[04-april-2023]]
== 4 avril 2023



[[support-for-china-regions-for-aws]]
=== Prise en charge des régions chinoises pour AWS

À partir de Cloud Volumes ONTAP 9.12.1 GA, les régions de Chine sont désormais prises en charge dans AWS comme suit.

* Les systèmes à nœud unique sont pris en charge.
* Les licences achetées directement auprès de NetApp sont prises en charge.


Pour connaître la disponibilité régionale, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Cartes des régions mondiales pour Cloud Volumes ONTAP"^] .



[[3-april-2023]]
== 3 avril 2023

Les modifications suivantes ont été introduites avec la version 3.9.28 du connecteur.



[[turin-region-now-supported-in-google-cloud]]
=== La région de Turin est désormais prise en charge dans Google Cloud

La région de Turin est désormais prise en charge dans Google Cloud pour Cloud Volumes ONTAP et Connector pour Cloud Volumes ONTAP 9.12.1 GA et versions ultérieures.



[[bluexp-digital-wallet-enhancement]]
=== Amélioration du BlueXP digital wallet

Le BlueXP digital wallet affiche désormais la capacité sous licence que vous avez achetée avec les offres privées du marché.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-capacity-licenses.html["Découvrez comment afficher la capacité consommée sur votre compte"^].



[[support-for-comments-during-volume-creation]]
=== Prise en charge des commentaires lors de la création du volume

Cette version vous permet de faire des commentaires lors de la création d'un volume Cloud Volumes ONTAP FlexGroup ou FlexVol volume lors de l'utilisation de l'API.



[[bluexp-user-interface-redesign-for-cloud-volumes-ontap-overview-volumes-and-aggregates-pages]]
=== Refonte de l'interface utilisateur BlueXP pour les pages Présentation, Volumes et Agrégats de Cloud Volumes ONTAP

BlueXP dispose désormais d'une interface utilisateur repensée pour les pages Présentation, Volumes et Agrégats de Cloud Volumes ONTAP .  La conception basée sur des tuiles présente des informations plus complètes dans chaque tuile pour une meilleure expérience utilisateur.

image:screenshot-resource-page-rn.png["Cette capture d'écran montre l'interface utilisateur BlueXP repensée sur la page de présentation de Cloud Volumes ONTAP .  Différentes tuiles affichent l'efficacité du stockage, la version, la distribution de la capacité, les informations sur le déploiement de Cloud Volumes ONTAP , les volumes, les agrégats, les réplications et les sauvegardes."]



[[flexgroup-volumes-viewable-through-cloud-volumes-ontap]]
=== Volumes FlexGroup visibles via Cloud Volumes ONTAP

Les volumes FlexGroup créés via ONTAP System Manager ou directement via l'interface de ligne de commande ONTAP sont désormais visibles via la mosaïque Volumes repensée dans BlueXP.  Identique aux informations fournies pour les volumes FlexVol , BlueXP fournit des informations détaillées sur les volumes FlexGroup créés via une mosaïque Volumes dédiée.


NOTE: Actuellement, vous ne pouvez afficher que les volumes FlexGroup existants sous BlueXP.  La possibilité de créer des volumes FlexGroup dans BlueXP n'est pas disponible mais prévue pour une future version.

image:screenshot-show-flexgroup-volume.png["Une capture d'écran qui montre le texte de survol de l'icône de volume FlexGroup sous la mosaïque Volumes."]

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-volumes.html["En savoir plus sur l’affichage des volumes FlexGroup créés."^]



[[13-march-2023]]
== 13 mars 2023



[[support-for-china-regions-in-azure]]
=== Prise en charge des régions chinoises dans Azure

La région Chine Nord 3 est désormais prise en charge pour les déploiements à nœud unique de Cloud Volumes ONTAP 9.12.1 GA et 9.13.0 GA dans Azure.  Seules les licences achetées directement auprès de NetApp (licences BYOL) sont prises en charge dans ces régions.


NOTE: Les nouveaux déploiements de Cloud Volumes ONTAP dans les régions de Chine sont pris en charge uniquement dans les versions 9.12.1 GA et 9.13.0 GA.  Vous pouvez mettre à niveau ces versions vers des correctifs et des versions ultérieurs de Cloud Volumes ONTAP.  Si vous souhaitez déployer des versions ultérieures de Cloud Volumes ONTAP dans les régions chinoises, contactez le support NetApp .

Pour connaître la disponibilité régionale, reportez-vous à la https://bluexp.netapp.com/cloud-volumes-global-regions["Cartes des régions mondiales pour Cloud Volumes ONTAP"^] .



[[05-march-2023]]
== 05 mars 2023

Les modifications suivantes ont été introduites avec la version 3.9.27 du connecteur.



[[cloud-volumes-ontap-9-13-0]]
=== Cloud Volumes ONTAP 9.13.0

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.13.0 dans AWS, Azure et Google Cloud.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9130-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[16-tib-and-32-tib-support-in-azure]]
=== Prise en charge de 16 Tio et 32 Tio dans Azure

Cloud Volumes ONTAP prend désormais en charge les tailles de disque de 16 Tio et 32 ​​Tio pour les déploiements haute disponibilité exécutés sur des disques gérés dans Azure.

En savoir plus sur https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html#supported-disk-sizes["tailles de disque prises en charge dans Azure"^] .



[[mtekm-license]]
=== Licence MTEKM

La licence Multi-tenant Encryption Key Management (MTEKM) est désormais incluse avec les systèmes Cloud Volumes ONTAP nouveaux et existants exécutant la version 9.12.1 GA ou ultérieure.

La gestion des clés externes multi-locataires permet aux machines virtuelles de stockage individuelles (SVM) de conserver leurs propres clés via un serveur KMIP lors de l'utilisation de NetApp Volume Encryption.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-encrypting-volumes.html["Découvrez comment chiffrer des volumes avec les solutions de chiffrement NetApp"^].



[[support-for-environments-without-internet]]
=== Prise en charge des environnements sans Internet

Cloud Volumes ONTAP est désormais pris en charge dans tout environnement cloud doté d'une isolation complète d'Internet.  Seules les licences basées sur les nœuds (BYOL) sont prises en charge dans ces environnements.  Les licences basées sur la capacité ne sont pas prises en charge.  Pour commencer, installez manuellement le logiciel Connector, connectez-vous à la console BlueXP exécutée sur le Connector, ajoutez votre licence BYOL au BlueXP digital wallet, puis déployez Cloud Volumes ONTAP.

* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-private-mode.html["Installer le connecteur dans un endroit sans accès Internet"^]
* https://docs.netapp.com/us-en/console-setup-admin/task-logging-in.html["Accéder à la console BlueXP sur le connecteur"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-node-licenses.html#manage-byol-licenses["Ajouter une licence non attribuée"^]




[[flash-cache-and-high-write-speed-in-google-cloud]]
=== Cache Flash et vitesse d'écriture élevée dans Google Cloud

La prise en charge du cache Flash, d'une vitesse d'écriture élevée et d'une unité de transmission maximale (MTU) élevée de 8 896 octets est désormais disponible pour certaines instances avec la version Cloud Volumes ONTAP 9.13.0.

En savoir plus sur https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-gcp.html["configurations prises en charge par licence pour Google Cloud"^] .



[[05-february-2023]]
== 05 février 2023

Les modifications suivantes ont été introduites avec la version 3.9.26 du connecteur.



[[placement-group-creation-in-aws]]
=== Création de groupe de placement dans AWS

Un nouveau paramètre de configuration est désormais disponible pour la création de groupes de placement avec les déploiements de zone de disponibilité unique (AZ) AWS HA.  Vous pouvez désormais choisir de contourner les échecs de création de groupes de placement et de permettre aux déploiements AWS HA mono-AZ de se terminer avec succès.

Pour plus d'informations sur la configuration du paramètre de création de groupe de placement, reportez-vous à https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-configure-placement-group-failure-aws.html#overview["Configurer la création d'un groupe de placement pour AWS HA Single AZ"^].



[[private-dns-zone-configuration-update]]
=== Mise à jour de la configuration de la zone DNS privée

Un nouveau paramètre de configuration est désormais disponible pour vous permettre d’éviter de créer un lien entre une zone DNS privée et un réseau virtuel lors de l’utilisation d’Azure Private Links.  La création est activée par défaut.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-enabling-private-link.html#provide-details-about-your-azure-private-dns["Fournissez des détails sur votre DNS privé Azure"^]



[[worm-storage-and-data-tiering]]
=== Stockage WORM et hiérarchisation des données

Vous pouvez désormais activer à la fois la hiérarchisation des données et le stockage WORM lorsque vous créez un système Cloud Volumes ONTAP 9.8 ou version ultérieure.  L'activation de la hiérarchisation des données avec le stockage WORM vous permet de hiérarchiser les données vers un magasin d'objets dans le cloud.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-worm.html["En savoir plus sur le stockage WORM."^]



[[1-january-2023]]
== 1er janvier 2023

Les modifications suivantes ont été introduites avec la version 3.9.25 du connecteur.



[[licensing-packages-available-in-google-cloud]]
=== Packages de licences disponibles dans Google Cloud

Des packages de licences optimisés et basés sur la capacité Edge Cache sont disponibles pour Cloud Volumes ONTAP sur Google Cloud Marketplace sous la forme d'une offre à la carte ou d'un contrat annuel.

Reportez-vous à  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#packages["Licences Cloud Volumes ONTAP"^].



[[default-configuration-for-cloud-volumes-ontap]]
=== Configuration par défaut pour Cloud Volumes ONTAP

La licence Multi-tenant Encryption Key Management (MTEKM) n'est plus incluse dans les nouveaux déploiements Cloud Volumes ONTAP .

Pour plus d'informations sur les licences de fonctionnalités ONTAP installées automatiquement avec Cloud Volumes ONTAP, reportez-vous à https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-default-configs.html["Configuration par défaut pour Cloud Volumes ONTAP"^].



[[15-december-2022]]
== 15 décembre 2022



[[cloud-volumes-ontap-9-12-0]]
=== Cloud Volumes ONTAP 9.12.0

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.12.0 dans AWS et Google Cloud.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9120-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[08-december-2022]]
== 08 décembre 2022



[[cloud-volumes-ontap-9-12-1]]
=== Cloud Volumes ONTAP 9.12.1

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.12.1, qui inclut la prise en charge de nouvelles fonctionnalités et de régions de fournisseurs de cloud supplémentaires.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9121-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^]



[[04-december-2022]]
== 4 décembre 2022

Les modifications suivantes ont été introduites avec la version 3.9.24 du connecteur.



[[worm-cloud-backup-now-available-during-cloud-volumes-ontap-creation]]
=== WORM + Cloud Backup désormais disponible lors de la création de Cloud Volumes ONTAP

La possibilité d'activer les fonctionnalités d'écriture unique et de lecture multiple (WORM) et de sauvegarde dans le cloud est désormais disponible pendant le processus de création de Cloud Volumes ONTAP .



[[israel-region-now-supported-in-google-cloud]]
=== La région d'Israël est désormais prise en charge dans Google Cloud

La région Israël est désormais prise en charge dans Google Cloud pour Cloud Volumes ONTAP et Connector pour Cloud Volumes ONTAP 9.11.1 P3 et versions ultérieures.



[[15-november-2022]]
== 15 novembre 2022

Les modifications suivantes ont été introduites avec la version 3.9.23 du connecteur.



[[ontap-s3-license-in-google-cloud]]
=== Licence ONTAP S3 dans Google Cloud

Une licence ONTAP S3 est désormais incluse sur les systèmes Cloud Volumes ONTAP nouveaux et existants exécutant la version 9.12.1 ou ultérieure dans Google Cloud Platform.

https://docs.netapp.com/us-en/ontap/object-storage-management/index.html["Documentation ONTAP : découvrez comment configurer et gérer les services de stockage d'objets S3"^]



[[06-november-2022]]
== 06 novembre 2022

Les modifications suivantes ont été introduites avec la version 3.9.23 du connecteur.



[[moving-resource-groups-in-azure]]
=== Déplacer des groupes de ressources dans Azure

Vous pouvez désormais déplacer un environnement de travail d’un groupe de ressources vers un autre groupe de ressources dans Azure au sein du même abonnement Azure.

Pour plus d'informations, consultez  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-moving-resource-groups-azure.html["Déplacer des groupes de ressources"].



[[ndmp-copy-certification]]
=== Certification de copie NDMP

NDMP-copy est désormais certifié pour une utilisation avec Cloud Volume ONTAP.

Pour plus d'informations sur la configuration et l'utilisation de NDMP, reportez-vous au https://docs.netapp.com/us-en/ontap/ndmp/index.html["Documentation ONTAP : Présentation de la configuration NDMP"] .



[[managed-disk-encryption-support-for-azure]]
=== Prise en charge du chiffrement de disque géré pour Azure

Une nouvelle autorisation Azure a été ajoutée qui vous permet désormais de chiffrer tous les disques gérés lors de leur création.

Pour plus d'informations sur cette nouvelle fonctionnalité, veuillez consulter https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-set-up-azure-encryption.html["Configurer Cloud Volumes ONTAP pour utiliser une clé gérée par le client dans Azure"].



[[18-september-2022]]
== 18 septembre 2022

Les modifications suivantes ont été introduites avec la version 3.9.22 du connecteur.



[[digital-wallet-enhancements]]
=== Améliorations du portefeuille numérique

* Le portefeuille numérique affiche désormais un résumé du package de licence d'E/S optimisé et de la capacité WORM provisionnée pour les systèmes Cloud Volumes ONTAP sur votre compte.
+
Ces détails peuvent vous aider à mieux comprendre comment vous êtes facturé et si vous devez acheter une capacité supplémentaire.

+
https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-capacity-licenses.html["Découvrez comment afficher la capacité consommée sur votre compte"].

* Vous pouvez désormais passer d'une méthode de charge à la méthode de charge optimisée.
+
https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-capacity-licenses.html["Apprenez à changer de méthode de charge"].





[[optimize-cost-and-performance]]
=== Optimiser les coûts et les performances

Vous pouvez désormais optimiser le coût et les performances d’un système Cloud Volumes ONTAP directement depuis le Canvas.

Après avoir sélectionné un environnement de travail, vous pouvez choisir l'option *Optimiser les coûts et les performances* pour modifier le type d'instance pour Cloud Volumes ONTAP.  Le choix d’une instance de plus petite taille peut vous aider à réduire les coûts, tandis que le passage à une instance de plus grande taille peut vous aider à optimiser les performances.



[[autosupport-notifications]]
=== Notifications AutoSupport

BlueXP générera désormais une notification si un système Cloud Volumes ONTAP ne parvient pas à envoyer de messages AutoSupport .  La notification inclut un lien vers des instructions que vous pouvez utiliser pour résoudre les problèmes de réseau.



[[31-july-2022]]
== 31 juillet 2022

Les modifications suivantes ont été introduites avec la version 3.9.21 du connecteur.



[[mtekm-license]]
=== Licence MTEKM

La licence Multi-tenant Encryption Key Management (MTEKM) est désormais incluse avec les systèmes Cloud Volumes ONTAP nouveaux et existants exécutant la version 9.11.1 ou ultérieure.

La gestion des clés externes multi-locataires permet aux machines virtuelles de stockage individuelles (SVM) de conserver leurs propres clés via un serveur KMIP lors de l'utilisation de NetApp Volume Encryption.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-encrypting-volumes.html["Découvrez comment chiffrer des volumes avec les solutions de chiffrement NetApp"].



[[proxy-server]]
=== Serveur proxy

BlueXP configure désormais automatiquement vos systèmes Cloud Volumes ONTAP pour utiliser le connecteur comme serveur proxy, si une connexion Internet sortante n'est pas disponible pour envoyer des messages AutoSupport .

AutoSupport surveille de manière proactive l’état de votre système et envoie des messages au support technique NetApp .

La seule exigence est de s'assurer que le groupe de sécurité du connecteur autorise les connexions _entrantes_ sur le port 3128.  Vous devrez ouvrir ce port après avoir déployé le connecteur.



[[change-charging-method]]
=== Changer la méthode de charge

Vous pouvez désormais modifier la méthode de facturation d'un système Cloud Volumes ONTAP qui utilise des licences basées sur la capacité.  Par exemple, si vous avez déployé un système Cloud Volumes ONTAP avec le package Essentials, vous pouvez le remplacer par le package Professional si les besoins de votre entreprise ont changé.  Cette fonctionnalité est disponible à partir du portefeuille numérique.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-capacity-licenses.html["Apprenez à changer de méthode de charge"].



[[security-group-enhancement]]
=== Amélioration du groupe de sécurité

Lorsque vous créez un environnement de travail Cloud Volumes ONTAP , l'interface utilisateur vous permet désormais de choisir si vous souhaitez que le groupe de sécurité prédéfini autorise le trafic au sein du réseau sélectionné uniquement (recommandé) ou de tous les réseaux.



[[18-july-2022]]
== 18 juillet 2022



[[new-licensing-packages-in-azure]]
=== Nouveaux packages de licences dans Azure

Deux nouveaux packages de licences basés sur la capacité sont disponibles pour Cloud Volumes ONTAP dans Azure lorsque vous payez via un abonnement Azure Marketplace :

* *Optimisé* : Payez séparément la capacité provisionnée et les opérations d'E/S
* *Edge Cache* : Licence pour https://bluexp.netapp.com/cloud-volumes-edge-cache["Cache Edge Cloud Volumes"^]


https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#packages["En savoir plus sur ces packages de licences"].



[[3-july-2022]]
== 3 juillet 2022

Les modifications suivantes ont été introduites avec la version 3.9.20 du connecteur.



[[digital-wallet]]
=== Portefeuille numérique

Le portefeuille numérique vous indique désormais la capacité totale consommée sur votre compte et la capacité consommée par package de licence.  Cela peut vous aider à comprendre comment vous êtes facturé et si vous devez acheter une capacité supplémentaire.



[[elastic-volumes-enhancement]]
=== Amélioration des volumes élastiques

BlueXP prend désormais en charge la fonctionnalité Amazon EBS Elastic Volumes lors de la création d'un environnement de travail Cloud Volumes ONTAP à partir de l'interface utilisateur.  La fonctionnalité Volumes élastiques est activée par défaut lors de l'utilisation de disques gp3 ou io1.  Vous pouvez choisir la capacité initiale en fonction de vos besoins de stockage et la réviser après le déploiement de Cloud Volumes ONTAP .

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-aws-elastic-volumes.html["En savoir plus sur la prise en charge des volumes élastiques dans AWS"].



[[ontap-s3-license-in-aws]]
=== Licence ONTAP S3 dans AWS

Une licence ONTAP S3 est désormais incluse sur les systèmes Cloud Volumes ONTAP nouveaux et existants exécutant la version 9.11.0 ou ultérieure dans AWS.

https://docs.netapp.com/us-en/ontap/object-storage-management/index.html["Documentation ONTAP : découvrez comment configurer et gérer les services de stockage d'objets S3"^]



[[new-azure-cloud-region-support]]
=== Prise en charge de la nouvelle région Azure Cloud

À partir de la version 9.10.1, Cloud Volumes ONTAP est désormais pris en charge dans la région Azure West US 3.

https://bluexp.netapp.com/cloud-volumes-global-regions["Consultez la liste complète des régions prises en charge pour Cloud Volumes ONTAP"^]



[[ontap-s3-license-in-azure]]
=== Licence ONTAP S3 dans Azure

Une licence ONTAP S3 est désormais incluse sur les systèmes Cloud Volumes ONTAP nouveaux et existants exécutant la version 9.9.1 ou ultérieure dans Azure.

https://docs.netapp.com/us-en/ontap/object-storage-management/index.html["Documentation ONTAP : découvrez comment configurer et gérer les services de stockage d'objets S3"^]



[[07-june-2022]]
== 07 juin 2022

Les modifications suivantes ont été introduites avec la version 3.9.19 du connecteur.



[[cloud-volumes-ontap-9-11-1]]
=== Cloud Volumes ONTAP 9.11.1

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.11.1, qui inclut la prise en charge de nouvelles fonctionnalités et de régions de fournisseurs de cloud supplémentaires.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9111-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^]



[[new-advanced-view]]
=== Nouvelle vue avancée

Si vous devez effectuer une gestion avancée de Cloud Volumes ONTAP, vous pouvez le faire à l'aide d' ONTAP System Manager, une interface de gestion fournie avec un système ONTAP .  Nous avons inclus l'interface du gestionnaire de système directement dans BlueXP afin que vous n'ayez pas besoin de quitter BlueXP pour une gestion avancée.

Cette vue avancée est disponible en tant qu'aperçu avec Cloud Volumes ONTAP 9.10.0 et versions ultérieures. Nous prévoyons d’affiner cette expérience et d’ajouter des améliorations dans les prochaines versions. Veuillez nous envoyer vos commentaires en utilisant le chat intégré au produit.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-administer-advanced-view.html["En savoir plus sur la vue avancée"].



[[support-for-amazon-ebs-elastic-volumes]]
=== Prise en charge des volumes élastiques Amazon EBS

La prise en charge de la fonctionnalité Amazon EBS Elastic Volumes avec un agrégat Cloud Volumes ONTAP offre de meilleures performances et une capacité supplémentaire, tout en permettant à BlueXP d'augmenter automatiquement la capacité du disque sous-jacent selon les besoins.

La prise en charge des volumes élastiques est disponible à partir des nouveaux systèmes Cloud Volumes ONTAP 9.11.0 et avec les types de disques EBS gp3 et io1.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-aws-elastic-volumes.html["En savoir plus sur la prise en charge des volumes élastiques"].

Notez que la prise en charge des volumes élastiques nécessite de nouvelles autorisations AWS pour le connecteur :

[source, json]
----
"ec2:DescribeVolumesModifications",
"ec2:ModifyVolume",
----
Veillez à accorder ces autorisations à chaque ensemble d'identifiants AWS que vous avez ajouté à BlueXP. https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-aws.html["Consultez la dernière politique de connecteur pour AWS"^]



[[support-for-deploying-ha-pairs-in-shared-aws-subnets]]
=== Prise en charge du déploiement de paires HA dans des sous-réseaux AWS partagés

Cloud Volumes ONTAP 9.11.1 inclut la prise en charge du partage AWS VPC.  Cette version du connecteur vous permet de déployer une paire HA dans un sous-réseau partagé AWS lors de l'utilisation de l'API.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-deploy-aws-shared-vpc.html["Découvrez comment déployer une paire HA dans un sous-réseau partagé"].



[[limited-network-access-when-using-service-endpoints]]
=== Accès réseau limité lors de l'utilisation de points de terminaison de service

BlueXP limite désormais l'accès au réseau lors de l'utilisation d'un point de terminaison de service VNet pour les connexions entre Cloud Volumes ONTAP et les comptes de stockage.  BlueXP utilise un point de terminaison de service si vous désactivez les connexions Azure Private Link.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-enabling-private-link.html["En savoir plus sur les connexions Azure Private Link avec Cloud Volumes ONTAP"].



[[support-for-creating-storage-vms-in-google-cloud]]
=== Prise en charge de la création de machines virtuelles de stockage dans Google Cloud

Plusieurs machines virtuelles de stockage sont désormais prises en charge avec Cloud Volumes ONTAP dans Google Cloud, à partir de la version 9.11.1.  À partir de cette version du connecteur, BlueXP vous permet de créer des machines virtuelles de stockage sur des paires Cloud Volumes ONTAP HA dans Google Cloud à l'aide de l'API.

La prise en charge de la création de machines virtuelles de stockage nécessite de nouvelles autorisations Google Cloud pour le connecteur :

[source, yaml]
----
- compute.instanceGroups.get
- compute.addresses.get
----
Notez que vous devez utiliser l'interface de ligne de commande ONTAP ou System Manager pour créer une machine virtuelle de stockage sur un système à nœud unique.

* https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-limits-gcp.html#storage-vm-limits["En savoir plus sur les limites des machines virtuelles de stockage dans Google Cloud"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-managing-svms-gcp.html["Découvrez comment créer des machines virtuelles de stockage de données pour Cloud Volumes ONTAP dans Google Cloud"]




[[02-may-2022]]
== 02 mai 2022

Les modifications suivantes ont été introduites avec la version 3.9.18 du connecteur.



[[cloud-volumes-ontap-9-11-0]]
=== Cloud Volumes ONTAP 9.11.0

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.11.0.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9110-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[enhancement-to-mediator-upgrades]]
=== Amélioration des mises à niveau du médiateur

Lorsque BlueXP met à niveau le médiateur pour une paire HA, il valide désormais qu'une nouvelle image de médiateur est disponible avant de supprimer le disque de démarrage.  Cette modification garantit que le médiateur peut continuer à fonctionner avec succès si le processus de mise à niveau échoue.



[[k8s-tab-has-been-removed]]
=== L'onglet K8s a été supprimé

L'onglet K8s était obsolète dans une version précédente et a maintenant été supprimé.



[[annual-contract-in-azure]]
=== Contrat annuel dans Azure

Les packages Essentials et Professional sont désormais disponibles dans Azure via un contrat annuel.  Vous pouvez contacter votre représentant commercial NetApp pour acheter un contrat annuel.  Le contrat est disponible sous forme d’offre privée sur la Place de marché Azure.

Une fois que NetApp a partagé l’offre privée avec vous, vous pouvez sélectionner le plan annuel lorsque vous vous abonnez à partir de la Place de marché Azure lors de la création de l’environnement de travail.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html["En savoir plus sur les licences"].



[[s3-glacier-instant-retrieval]]
=== Récupération instantanée du glacier S3

Vous pouvez désormais stocker des données hiérarchisées dans la classe de stockage Amazon Simple Storage Service (Amazon S3) Glacier Instant Retrieval.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-tiering.html#changing-the-storage-class-for-tiered-data["Découvrez comment modifier la classe de stockage des données hiérarchisées"].



[[new-aws-permissions-required-for-the-connector]]
=== Nouvelles autorisations AWS requises pour le connecteur

Les autorisations suivantes sont désormais requises pour créer un groupe de placement réparti AWS lors du déploiement d'une paire HA dans une seule zone de disponibilité (AZ) :

[source, json]
----
"ec2:DescribePlacementGroups",
"iam:GetRolePolicy",
----
Ces autorisations sont désormais nécessaires pour optimiser la manière dont BlueXP crée le groupe de placement.

Veillez à accorder ces autorisations à chaque ensemble d'identifiants AWS que vous avez ajouté à BlueXP. https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-aws.html["Consultez la dernière politique de connecteur pour AWS"^]



[[new-google-cloud-region-support]]
=== Nouvelle prise en charge régionale de Google Cloud

Cloud Volumes ONTAP est désormais pris en charge dans les régions Google Cloud suivantes à partir de la version 9.10.1 :

* Delhi (Asie-Sud2)
* Melbourne (Australie-sud-est2)
* Milan (europe-west8) - nœud unique uniquement
* Santiago (southamerica-west1) - nœud unique uniquement


https://bluexp.netapp.com/cloud-volumes-global-regions["Consultez la liste complète des régions prises en charge pour Cloud Volumes ONTAP"^]



[[support-for-n2-standard-16-in-google-cloud]]
=== Prise en charge de n2-standard-16 dans Google Cloud

Le type de machine n2-standard-16 est désormais pris en charge avec Cloud Volumes ONTAP dans Google Cloud, à partir de la version 9.10.1.

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-gcp.html["Afficher les configurations prises en charge pour Cloud Volumes ONTAP dans Google Cloud"^]



[[enhancements-to-google-cloud-firewall-policies]]
=== Améliorations apportées aux stratégies de pare-feu de Google Cloud

* Lorsque vous créez une paire Cloud Volumes ONTAP HA dans Google Cloud, BlueXP affiche désormais toutes les stratégies de pare-feu existantes dans un VPC.
+
Auparavant, BlueXP n’affichait aucune stratégie dans VPC-1, VPC-2 ou VPC-3 qui n’avait pas de balise cible.

* Lorsque vous créez un système Cloud Volumes ONTAP à nœud unique dans Google Cloud, vous pouvez désormais choisir si vous souhaitez que la politique de pare-feu prédéfinie autorise le trafic uniquement au sein du VPC sélectionné (recommandé) ou dans tous les VPC.




[[enhancement-to-google-cloud-service-accounts]]
=== Amélioration des comptes de service Google Cloud

Lorsque vous sélectionnez le compte de service Google Cloud à utiliser avec Cloud Volumes ONTAP, BlueXP affiche désormais l'adresse e-mail associée à chaque compte de service.  L’affichage de l’adresse e-mail peut faciliter la distinction entre les comptes de service qui partagent le même nom.



[[3-april-2022]]
== 3 avril 2022



[[system-manager-link-has-been-removed]]
=== Le lien vers le gestionnaire de système a été supprimé

Nous avons supprimé le lien System Manager qui était auparavant disponible dans un environnement de travail Cloud Volumes ONTAP .

Vous pouvez toujours vous connecter à System Manager en saisissant l'adresse IP de gestion du cluster dans un navigateur web ayant une connexion au système Cloud Volumes ONTAP. https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-connecting-to-otc.html["En savoir plus sur la connexion au Gestionnaire système"]



[[charging-for-worm-storage]]
=== Frais de stockage WORM

Maintenant que le tarif spécial de lancement a expiré, vous serez facturé pour l'utilisation du stockage WORM.  La facturation est horaire, en fonction de la capacité totale provisionnée des volumes WORM.  Ceci s'applique aux systèmes Cloud Volumes ONTAP nouveaux et existants.

https://bluexp.netapp.com/pricing["En savoir plus sur les tarifs du stockage WORM"^] .



[[27-february-2022]]
== 27 février 2022

Les modifications suivantes ont été introduites avec la version 3.9.16 du connecteur.



[[redesigned-volume-wizard]]
=== Assistant de volume repensé

L'assistant de création de nouveau volume que nous avons récemment introduit est désormais disponible lors de la création d'un volume sur un agrégat spécifique à partir de l'option *Allocation avancée*.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-create-volumes.html["Apprenez à créer des volumes sur un agrégat spécifique"].



[[09-february-2022]]
== 09 février 2022



[[marketplace-updates]]
=== Mises à jour du marché

* Les packages Essentials et Professional sont désormais disponibles sur toutes les places de marché des fournisseurs de cloud.
+
Ces méthodes de facturation par capacité vous permettent de payer à l'heure ou d'acheter un contrat annuel directement auprès de votre fournisseur de cloud.  Vous avez toujours la possibilité d’acheter une licence par capacité directement auprès de NetApp.

+
Si vous disposez déjà d'un abonnement sur une place de marché cloud, vous êtes également automatiquement abonné à ces nouvelles offres.  Vous pouvez choisir la facturation par capacité lorsque vous déployez un nouvel environnement de travail Cloud Volumes ONTAP .

+
Si vous êtes un nouveau client, BlueXP vous demandera de vous abonner lorsque vous créerez un nouvel environnement de travail.

* Les licences par nœud de tous les marchés de fournisseurs de cloud sont obsolètes et ne sont plus disponibles pour les nouveaux abonnés.  Cela comprend les contrats annuels et les abonnements horaires (Explore, Standard et Premium).
+
Cette méthode de facturation est toujours disponible pour les clients existants qui ont un abonnement actif.



https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html["En savoir plus sur les options de licence pour Cloud Volumes ONTAP"].



[[06-february-2022]]
== 06 février 2022



[[exchange-unassigned-licenses]]
=== Échanger des licences non attribuées

Si vous disposez d'une licence basée sur un nœud non attribué pour Cloud Volumes ONTAP que vous n'avez pas utilisée, vous pouvez désormais échanger la licence en la convertissant en licence Cloud Backup, en licence Cloud Data Sense ou en licence Cloud Tiering.

Cette action révoque la licence Cloud Volumes ONTAP et crée une licence équivalente en dollars pour le service avec la même date d'expiration.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-node-licenses.html#exchange-unassigned-node-based-licenses["Découvrez comment échanger des licences basées sur des nœuds non attribuées"].



[[30-january-2022]]
== 30 janvier 2022

Les modifications suivantes ont été introduites avec la version 3.9.15 du connecteur.



[[redesigned-licensing-selection]]
=== Sélection de licences repensée

Nous avons repensé l'écran de sélection de licence lors de la création d'un nouvel environnement de travail Cloud Volumes ONTAP .  Les changements mettent en évidence les méthodes de facturation par capacité qui ont été introduites en juillet 2021 et prennent en charge les offres à venir via les marchés des fournisseurs de cloud.



[[digital-wallet-update]]
=== Mise à jour du portefeuille numérique

Nous avons mis à jour le *Portefeuille numérique* en consolidant les licences Cloud Volumes ONTAP dans un seul onglet.



[[02-january-2022]]
== 02 janvier 2022

Les modifications suivantes ont été introduites avec la version 3.9.14 du connecteur.



[[support-for-additional-azure-vm-types]]
=== Prise en charge de types de machines virtuelles Azure supplémentaires

Cloud Volumes ONTAP est désormais pris en charge avec les types de machines virtuelles suivants dans Microsoft Azure, à partir de la version 9.10.1 :

* E4ds_v4
* E8ds_v4
* E32ds_v4
* E48ds_v4


Aller à la https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes["Notes de version de Cloud Volumes ONTAP"^] pour plus de détails sur les configurations prises en charge.



[[flexclone-charging-update]]
=== Mise à jour de chargement de FlexClone

Si vous utilisez un  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html["licence basée sur la capacité"^] pour Cloud Volumes ONTAP, vous n'êtes plus facturé pour la capacité utilisée par les volumes FlexClone.



[[charging-method-now-displayed]]
=== La méthode de charge est désormais affichée

BlueXP affiche désormais la méthode de facturation pour chaque environnement de travail Cloud Volumes ONTAP dans le panneau de droite du canevas.

image:screenshot-cvo-charging-method.png["Une capture d'écran qui montre la méthode de facturation pour un environnement de travail Cloud Volumes ONTAP qui apparaît dans le panneau de droite après avoir sélectionné un environnement de travail dans le canevas."]



[[choose-your-user-name]]
=== Choisissez votre nom d'utilisateur

Lorsque vous créez un environnement de travail Cloud Volumes ONTAP , vous avez désormais la possibilité de saisir votre nom d'utilisateur préféré, au lieu du nom d'utilisateur administrateur par défaut.

image:screenshot-cvo-user-name.png["Une capture d’écran de la page Détails et informations d’identification dans l’assistant d’environnement de travail où vous pouvez spécifier un nom d’utilisateur."]



[[volume-creation-enhancements]]
=== Améliorations de la création de volumes

Nous avons apporté quelques améliorations à la création de volumes :

* Nous avons repensé l’assistant de création de volume pour plus de simplicité d’utilisation.
* Vous pouvez désormais choisir une politique d’exportation personnalisée pour NFS.


image:screenshot-cvo-create-volume.png["Une capture d’écran qui montre la page Protocole lors de la création d’un nouveau volume."]



[[28-november-2021]]
== 28 novembre 2021

Les modifications suivantes ont été introduites avec la version 3.9.13 du connecteur.



[[cloud-volumes-ontap-9-10-1]]
=== Cloud Volumes ONTAP 9.10.1

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.10.1.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9101-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[netapp-keystone-subscriptions]]
=== Abonnements NetApp Keystone

Vous pouvez désormais utiliser les abonnements Keystone pour payer les paires Cloud Volumes ONTAP HA.

Un abonnement Keystone est un service d'abonnement à paiement progressif qui offre une expérience de cloud hybride transparente pour ceux qui préfèrent les modèles de consommation OpEx aux CapEx initiaux ou à la location.

Un abonnement Keystone est pris en charge avec toutes les nouvelles versions de Cloud Volumes ONTAP que vous pouvez déployer à partir de BlueXP.

* https://www.netapp.com/services/keystone/["En savoir plus sur les abonnements NetApp Keystone"^] .
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-manage-keystone.html["Découvrez comment démarrer avec les abonnements Keystone dans BlueXP"^].




[[new-aws-region-support]]
=== Nouvelle prise en charge régionale AWS

Cloud Volumes ONTAP est désormais pris en charge dans la région AWS Asie-Pacifique (Osaka) (ap-northeast-3).



[[port-reduction]]
=== Réduction du port

Les ports 8023 et 49000 ne sont plus ouverts sur les systèmes Cloud Volumes ONTAP dans Azure, que ce soit pour les systèmes à nœud unique ou les paires haute disponibilité.

Cette modification s’applique aux _nouveaux_ systèmes Cloud Volumes ONTAP à partir de la version 3.9.13 du connecteur.



[[04-october-2021]]
== 4 octobre 2021

Les modifications suivantes ont été introduites avec la version 3.9.11 du connecteur.



[[cloud-volumes-ontap-9-10-0]]
=== Cloud Volumes ONTAP 9.10.0

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.10.0.

https://docs.netapp.com/us-en/cloud-volumes-ontap-9100-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[reduced-deployment-time]]
=== Temps de déploiement réduit

Nous avons réduit le temps nécessaire au déploiement d’un environnement de travail Cloud Volumes ONTAP dans Microsoft Azure ou dans Google Cloud lorsque la vitesse d’écriture normale est activée.  Le temps de déploiement est désormais 3 à 4 minutes plus court en moyenne.



[[02-september-2021]]
== 02 septembre 2021

Les modifications suivantes ont été introduites avec la version 3.9.10 du connecteur.



[[customer-managed-encryption-key-in-azure]]
=== Clé de chiffrement gérée par le client dans Azure

Les données sont automatiquement chiffrées sur Cloud Volumes ONTAP dans Azure à l'aide de https://learn.microsoft.com/en-us/azure/security/fundamentals/encryption-overview["Chiffrement du service de stockage Azure"^] avec une clé gérée par Microsoft.  Mais vous pouvez désormais utiliser votre propre clé de chiffrement gérée par le client en suivant les étapes suivantes :

. Depuis Azure, créez un coffre de clés, puis générez une clé dans ce coffre.
. Depuis BlueXP, utilisez l’API pour créer un environnement de travail Cloud Volumes ONTAP qui utilise la clé.


https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-set-up-azure-encryption.html["En savoir plus sur ces étapes"].



[[07-july-2021]]
== 07 juillet 2021

Les modifications suivantes ont été introduites avec la version 3.9.8 du connecteur.



[[new-charging-methods]]
=== Nouvelles méthodes de charge

De nouvelles méthodes de facturation sont disponibles pour Cloud Volumes ONTAP.

* *BYOL basé sur la capacité* : une licence basée sur la capacité vous permet de payer Cloud Volumes ONTAP par Tio de capacité.  La licence est associée à votre compte NetApp et vous permet de créer plusieurs systèmes Cloud Volumes ONTAP , à condition qu'une capacité suffisante soit disponible via votre licence.  Les licences basées sur la capacité sont disponibles sous la forme d'un package, soit _Essentials_ ou _Professional_.
* *Offre Freemium* : Freemium vous permet d'utiliser gratuitement toutes les fonctionnalités de Cloud Volumes ONTAP de NetApp (des frais de fournisseur de cloud s'appliquent toujours).  Vous êtes limité à 500 Gio de capacité provisionnée par système et il n'y a pas de contrat de support.  Vous pouvez avoir jusqu'à 10 systèmes Freemium.
+
https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html["En savoir plus sur ces options de licence"].

+
Voici un exemple des méthodes de facturation parmi lesquelles vous pouvez choisir :

+
image:screenshot_cvo_charging_methods.png["Une capture d'écran de l'assistant d'environnement de travail Cloud Volumes ONTAP où vous pouvez choisir une méthode de facturation."]





[[worm-storage-available-for-general-use]]
=== Stockage WORM disponible pour une utilisation générale

Le stockage WORM (Write Once, Read Many) n'est plus en préversion et est désormais disponible pour une utilisation générale avec Cloud Volumes ONTAP. https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-worm.html["En savoir plus sur le stockage WORM"].



[[support-for-m5dn-24xlarge-in-aws]]
=== Prise en charge de m5dn.24xlarge dans AWS

À partir de la version 9.9.1, Cloud Volumes ONTAP prend désormais en charge le type d'instance m5dn.24xlarge avec les méthodes de facturation suivantes : PAYGO Premium, Bring-Your-Own License (BYOL) et Freemium.

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-aws.html["Afficher les configurations prises en charge pour Cloud Volumes ONTAP dans AWS"^] .



[[select-existing-azure-resource-groups]]
=== Sélectionner des groupes de ressources Azure existants

Lors de la création d’un système Cloud Volumes ONTAP dans Azure, vous avez désormais la possibilité de sélectionner un groupe de ressources existant pour la machine virtuelle et ses ressources associées.

image:screenshot_azure_resource_group.png["Une capture d’écran de l’assistant Créer un environnement de travail où vous pouvez sélectionner un groupe de ressources existant."]

Les autorisations suivantes permettent à BlueXP de supprimer les ressources Cloud Volumes ONTAP d'un groupe de ressources, en cas d'échec de déploiement ou de suppression :

[source, json]
----
"Microsoft.Network/privateEndpoints/delete",
"Microsoft.Compute/availabilitySets/delete",
----
Veillez à accorder ces autorisations à chaque ensemble d'informations d'identification Azure que vous avez ajouté à BlueXP. https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["Consultez la dernière politique de connecteur pour Azure"^].



[[blob-public-access-now-disabled-in-azure]]
=== L'accès public aux blobs est désormais désactivé dans Azure

En tant qu'amélioration de sécurité, BlueXP désactive désormais l'*accès public Blob* lors de la création d'un compte de stockage pour Cloud Volumes ONTAP.



[[azure-private-link-enhancement]]
=== Amélioration d'Azure Private Link

Par défaut, BlueXP active désormais une connexion Azure Private Link sur le compte de stockage de diagnostics de démarrage pour les nouveaux systèmes Cloud Volumes ONTAP .

Cela signifie que _tous_ les comptes de stockage pour Cloud Volumes ONTAP utiliseront désormais un lien privé.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-enabling-private-link.html["En savoir plus sur l'utilisation d'un lien privé Azure avec Cloud Volumes ONTAP"].



[[balanced-persistent-disks-in-google-cloud]]
=== Disques persistants équilibrés dans Google Cloud

À partir de la version 9.9.1, Cloud Volumes ONTAP prend désormais en charge les disques persistants équilibrés (pd-balanced).

Ces SSD équilibrent performances et coût en fournissant des IOPS par Gio inférieurs.



[[custom-4-16384-no-longer-supported-in-google-cloud]]
=== custom-4-16384 n'est plus pris en charge dans Google Cloud

Le type de machine custom-4-16384 n'est plus pris en charge avec les nouveaux systèmes Cloud Volumes ONTAP .

Si vous disposez déjà d'un système fonctionnant sur ce type de machine, vous pouvez continuer à l'utiliser, mais nous vous recommandons de passer au type de machine n2-standard-4.

https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-gcp.html["Afficher les configurations prises en charge pour Cloud Volumes ONTAP dans Google Cloud"^].



[[30-may-2021]]
== 30 mai 2021

Les modifications suivantes ont été introduites avec la version 3.9.7 du connecteur.



[[new-professional-package-in-aws]]
=== Nouveau package professionnel dans AWS

Un nouveau package professionnel vous permet de regrouper Cloud Volumes ONTAP et Cloud Backup Service en utilisant un contrat annuel de la place de marché AWS. Le paiement s'effectue par Tio. Cet abonnement ne vous permet pas de sauvegarder les données sur site.

Si vous choisissez cette option de paiement, vous pouvez provisionner jusqu'à 2 PiB par système Cloud Volumes ONTAP via des disques EBS et une hiérarchisation vers le stockage d'objets S3 (nœud unique ou HA).

Aller à la https://aws.amazon.com/marketplace/pp/prodview-q7dg6zwszplri["Page AWS Marketplace"^] pour afficher les détails des prix et accéder à la https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes["Notes de version de Cloud Volumes ONTAP"^] pour en savoir plus sur cette option de licence.



[[tags-on-ebs-volumes-in-aws]]
=== Balises sur les volumes EBS dans AWS

BlueXP ajoute désormais des balises aux volumes EBS lorsqu'il crée un nouvel environnement de travail Cloud Volumes ONTAP .  Les balises ont été créées précédemment après le déploiement de Cloud Volumes ONTAP .

Ce changement peut être utile si votre organisation utilise des politiques de contrôle des services (SCP) pour gérer les autorisations.



[[minimum-cooling-period-for-auto-tiering-policy]]
=== Période de refroidissement minimale pour la politique de hiérarchisation automatique

Si vous avez activé la hiérarchisation des données sur un volume à l’aide de la stratégie de hiérarchisation _auto_, vous pouvez désormais ajuster la période de refroidissement minimale à l’aide de l’API.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-tiering.html#changing-the-cooling-period-for-the-auto-tiering-policy["Apprenez à régler la période de refroidissement minimale."]



[[enhancement-to-custom-export-policies]]
=== Amélioration des politiques d'exportation personnalisées

Lorsque vous créez un nouveau volume NFS, BlueXP affiche désormais les stratégies d'exportation personnalisées par ordre croissant, ce qui vous permet de trouver plus facilement la stratégie d'exportation dont vous avez besoin.



[[deletion-of-old-cloud-snapshots]]
=== Suppression des anciens instantanés du cloud

BlueXP supprime désormais les anciens snapshots cloud des disques racine et de démarrage créés lors du déploiement d'un système Cloud Volumes ONTAP et à chaque mise hors tension.  Seuls les deux instantanés les plus récents sont conservés pour les volumes racine et de démarrage.

Cette amélioration permet de réduire les coûts des fournisseurs de cloud en supprimant les instantanés qui ne sont plus nécessaires.

Notez qu'un connecteur nécessite une nouvelle autorisation pour supprimer les instantanés Azure. https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["Consultez la dernière politique de connecteur pour Azure"^].

[source, json]
----
"Microsoft.Compute/snapshots/delete"
----


[[24-may-2021]]
== 24 mai 2021



[[cloud-volumes-ontap-9-9-1]]
=== Cloud Volumes ONTAP 9.9.1

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.9.1.

https://docs.netapp.com/us-en/cloud-volumes-ontap-991-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[11-apr-2021]]
== 11 avril 2021

Les modifications suivantes ont été introduites avec la version 3.9.5 du connecteur.



[[logical-space-reporting]]
=== Rapports d'espace logique

BlueXP permet désormais de générer des rapports d’espace logique sur la machine virtuelle de stockage initiale qu’il crée pour Cloud Volumes ONTAP.

Lorsque l'espace est signalé de manière logique, ONTAP signale l'espace du volume de telle sorte que tout l'espace physique économisé par les fonctionnalités d'efficacité de stockage soit également signalé comme utilisé.



[[support-for-gp3-disks-in-aws]]
=== Prise en charge des disques gp3 dans AWS

Cloud Volumes ONTAP prend désormais en charge les disques SSD à usage général (gp3) à partir de la version 9.7. Les disques gp3 sont les SSD les moins chers qui équilibrent coût et performances pour une large gamme de charges de travail.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html["Dimensionnez votre système dans AWS"].



[[cold-hdd-disks-no-longer-supported-in-aws]]
=== Les disques durs froids ne sont plus pris en charge dans AWS

Cloud Volumes ONTAP ne prend plus en charge les disques Cold HDD (sc1).



[[tls-1-2-for-azure-storage-accounts]]
=== TLS 1.2 pour les comptes de stockage Azure

Lorsque BlueXP crée des comptes de stockage dans Azure pour Cloud Volumes ONTAP, la version TLS du compte de stockage est désormais la version 1.2.



[[08-mar-2021]]
== 08 mars 2021

Les modifications suivantes ont été introduites avec la version 3.9.4 du connecteur.



[[cloud-volumes-ontap-9-9-0]]
=== Cloud Volumes ONTAP 9.9.0

BlueXP peut désormais déployer et gérer Cloud Volumes ONTAP 9.9.0.

https://docs.netapp.com/us-en/cloud-volumes-ontap-990-relnotes["Découvrez les nouvelles fonctionnalités incluses dans cette version de Cloud Volumes ONTAP"^] .



[[support-for-the-aws-c2s-environment]]
=== Prise en charge de l'environnement AWS C2S

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.8 dans l'environnement AWS Commercial Cloud Services (C2S).

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-getting-started-aws-c2s.html["Déployer Cloud Volumes ONTAP dans AWS Secret Cloud ou AWS Top Secret Cloud"].



[[aws-encryption-with-customer-managed-cmks]]
=== Chiffrement AWS avec CMK gérés par le client

BlueXP vous a toujours permis de crypter les données Cloud Volumes ONTAP à l'aide d'AWS Key Management Service (KMS).  À partir de Cloud Volumes ONTAP 9.9.0, les données sur les disques EBS et les données hiérarchisées sur S3 sont chiffrées si vous sélectionnez une CMK gérée par le client.  Auparavant, seules les données EBS étaient cryptées.

Notez que vous devrez fournir au rôle IAM Cloud Volumes ONTAP un accès pour utiliser la CMK.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["En savoir plus sur la configuration d'AWS KMS avec Cloud Volumes ONTAP"].



[[support-for-azure-dod]]
=== Prise en charge d'Azure DoD

Vous pouvez désormais déployer Cloud Volumes ONTAP 9.8 dans le niveau d’impact 6 (IL6) d’Azure Department of Defense (DoD).



[[ip-address-reduction-in-google-cloud]]
=== Réduction de l'adresse IP dans Google Cloud

Nous avons réduit le nombre d'adresses IP requises pour Cloud Volumes ONTAP 9.8 et versions ultérieures dans Google Cloud.  Par défaut, une adresse IP de moins est requise (nous avons unifié le LIF intercluster avec le LIF de gestion des nœuds).  Vous avez également la possibilité d'ignorer la création du LIF de gestion SVM lors de l'utilisation de l'API, ce qui réduirait le besoin d'une adresse IP supplémentaire.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-cloud-volumes-ontap["En savoir plus sur les exigences en matière d'adresse IP dans Google Cloud"].



[[shared-vpc-support-in-google-cloud]]
=== Prise en charge du VPC partagé dans Google Cloud

Lorsque vous déployez une paire Cloud Volumes ONTAP HA dans Google Cloud, vous pouvez désormais choisir des VPC partagés pour VPC-1, VPC-2 et VPC-3.  Auparavant, seul VPC-0 pouvait être un VPC partagé.  Cette modification est prise en charge avec Cloud Volumes ONTAP 9.8 et versions ultérieures.

https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html["En savoir plus sur les exigences réseau de Google Cloud"].



[[04-january-2021]]
== 04 janvier 2021

Les modifications suivantes ont été introduites avec la version 3.9.2 du connecteur.



[[aws-outposts]]
=== Avant-postes AWS

Il y a quelques mois, nous avons annoncé que Cloud Volumes ONTAP avait obtenu la désignation Amazon Web Services (AWS) Outposts Ready.  Aujourd'hui, nous sommes heureux d'annoncer que nous avons validé BlueXP et Cloud Volumes ONTAP avec AWS Outposts.

Si vous disposez d'un Outpost AWS, vous pouvez déployer Cloud Volumes ONTAP dans cet Outpost en sélectionnant le VPC Outpost dans l'assistant Environnement de travail. L'expérience est la même que pour tout autre VPC résidant dans AWS. Notez que vous devrez d’abord déployer un connecteur dans votre AWS Outpost.

Il y a quelques limitations à souligner :

* Seuls les systèmes Cloud Volumes ONTAP à nœud unique sont actuellement pris en charge
* Les instances EC2 que vous pouvez utiliser avec Cloud Volumes ONTAP sont limitées à ce qui est disponible dans votre Outpost
* Seuls les SSD à usage général (gp2) sont actuellement pris en charge




[[ultra-ssd-vnvram-in-supported-azure-regions]]
=== Ultra SSD VNVRAM dans les régions Azure prises en charge

Cloud Volumes ONTAP peut désormais utiliser un SSD Ultra comme VNVRAM lorsque vous utilisez le type de VM E32s_v3 avec un système à nœud unique https://docs.microsoft.com/en-us/azure/virtual-machines/disks-enable-ultra-ssd["dans n'importe quelle région Azure prise en charge"^].

La VNVRAM offre de meilleures performances d'écriture.



[[choose-an-availability-zone-in-azure]]
=== Choisir une zone de disponibilité dans Azure

Vous pouvez désormais choisir la zone de disponibilité dans laquelle vous souhaitez déployer un système Cloud Volumes ONTAP à nœud unique.  Si vous ne sélectionnez pas d'AZ, BlueXP en sélectionnera un pour vous.

image:screenshot_azure_az.gif["Une capture d'écran de la liste déroulante Zone de disponibilité disponible après avoir choisi une région."]



[[larger-disks-in-google-cloud]]
=== Disques plus grands dans Google Cloud

Cloud Volumes ONTAP prend désormais en charge les disques de 64 TB dans Google Cloud.


NOTE: La capacité maximale du système avec les seuls disques reste à 256 TB en raison des limites de Google Cloud.



[[new-machine-types-in-google-cloud]]
=== Nouveaux types de machines dans Google Cloud

Cloud Volumes ONTAP prend désormais en charge les types de machines suivants avec ces licences PAYGO dans Google Cloud :

* n2-standard-4 avec la licence Explore et avec BYOL
* n2-standard-8 avec la licence Standard et avec BYOL
* n2-standard-32 avec la licence Premium et avec BYOL




[[03-nov-2020]]
== 03 novembre 2020

Les modifications suivantes ont été introduites avec la version 3.9.0 du connecteur.



[[azure-private-link-for-cloud-volumes-ontap]]
=== Azure Private Link pour Cloud Volumes ONTAP

Par défaut, BlueXP active désormais une connexion Azure Private Link entre Cloud Volumes ONTAP et ses comptes de stockage associés.  Un lien privé sécurise les connexions entre les points de terminaison dans Azure.

* https://docs.microsoft.com/en-us/azure/private-link/private-link-overview["En savoir plus sur Azure Private Links"^]
* https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-enabling-private-link.html["En savoir plus sur l'utilisation d'un lien privé Azure avec Cloud Volumes ONTAP"^]

