...
+
** Vous devriez avoir un https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Agent de console associé à votre système"^].
** https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["Vous devez être prêt à laisser l'agent de la console en cours d'exécution à tout moment."^].
** Le compte de service associé à l’agent NetApp Console https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html["devrait avoir les autorisations requises"^].
...
. Si vous y êtes invité https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["créer un agent de console"^].
...
| Nom du compte de service | Si vous prévoyez d'utiliser link:concept-data-tiering.html["hiérarchisation des données"] ou  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^] avec Cloud Volumes ONTAP, vous devez activer le *compte de service* et sélectionner un compte de service disposant du rôle prédéfini Storage Admin. link:task-creating-gcp-service-account.html["Apprenez à créer un compte de service"^]. 
...
NOTE: Il s'agit du compte de service que vous avez configuré pour la Console, link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["comme décrit sur cette page"^].

Cliquez sur *Ajouter un abonnement* pour associer les informations d’identification sélectionnées à un abonnement.

Pour créer un système Cloud Volumes ONTAP à paiement à l'usage, vous devez sélectionner un projet Google Cloud associé à un abonnement à Cloud Volumes ONTAP depuis la place de marché Google Cloud. Consultez  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Associer un abonnement Marketplace aux identifiants Google Cloud"^].
...
. *Emplacement et connectivité* : Sélectionnez la région et la zone Google Cloud pour votre système, choisissez une politique de pare-feu et confirmez la connectivité réseau au stockage Google Cloud pour la hiérarchisation des données.
+
Le tableau suivant décrit les domaines pour lesquels vous pourriez avoir besoin de conseils :
...
. *Packages préconfigurés* : Sélectionnez l’un des packages pour déployer rapidement un système Cloud Volumes ONTAP, ou cliquez sur *Créer ma propre configuration*. Les packages préconfigurés varient selon la version Cloud Volumes ONTAP sélectionnée. Par exemple, pour Cloud Volumes ONTAP 9.18.1 et versions ultérieures, la Console affiche des packages avec des machines virtuelles C3, y compris des disques Hyperdisk Balanced. Vous pouvez modifier les configurations, telles que les paramètres d’IOPS et de débit, en fonction des besoins de votre charge de travail.
+
...
+
Pour obtenir de l'aide concernant le choix du type et de la taille du disque, reportez-vous à link:task-planning-your-config-gcp.html#size-your-system-in-google-cloud["Dimensionnez votre système dans Google Cloud"^].

. *Cache Flash, vitesse d'écriture et WORM* :
+
.. Activez *Flash Cache* ou choisissez *Normal* ou *High* comme vitesse d'écriture si vous en avez besoin.
+
Apprenez-en davantage sur link:reference-configs-gcp.html#flash-cache-support["Cache Flash"] et link:concept-write-speed.html["vitesse d'écriture"].

+

NOTE: L'option *Vitesse d'écriture élevée* permet de bénéficier d'une vitesse d'écriture élevée et d'une unité de transmission maximale (MTU) de 8 896 octets. De plus, la MTU plus élevée de 8 896 octets nécessite la sélection des VPC-1, VPC-2 et VPC-3 pour le déploiement. Pour plus d'informations sur les VPC-1, VPC-2 et VPC-3, veuillez consulter  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-console-agent["Règles pour VPC-1, VPC-2 et VPC-3"^].
...
** Pour obtenir de l'aide concernant l'ajout d'un compte Google Cloud, veuillez consulter  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Configuration et ajout de comptes Google Cloud pour la hiérarchisation des données avec 9.6"^].
...
| Serveur NTP | Sélectionnez *Utiliser le domaine Active Directory* pour configurer un serveur NTP à l'aide du DNS Active Directory. Si vous devez configurer un serveur NTP avec une adresse différente, vous devez utiliser l'API. Pour plus d'informations, consultez la  https://docs.netapp.com/us-en/console-automation/index.html["Documentation sur l'automatisation de la NetApp Console"^] pour plus de détails. Notez que vous pouvez configurer un serveur NTP uniquement lors de la création d'un serveur CIFS. Il n'est plus possible de le configurer après la création du serveur CIFS. 
|===
...
. *Détails du système* : Sélectionnez un projet, spécifiez un nom de cluster, sélectionnez éventuellement un compte de service, ajoutez éventuellement des étiquettes, puis spécifiez les informations d’identification.
+
Le tableau suivant décrit les domaines pour lesquels vous pourriez avoir besoin de conseils :
...
| Projet Google Cloud et abonnement Marketplace  a| 
Sélectionnez le projet Google et l’abonnement que vous souhaitez utiliser pour ce système. Il s’agit du compte de service que vous avez configuré pour la Console. Pour plus d’informations, consultez la link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Documents d'installation de la console"^]. La Console utilise le projet pour créer le système Cloud Volumes ONTAP et l’abonnement pour déterminer la facturation et l’utilisation. Si aucun autre projet n’apparaît dans la liste déroulante, cela signifie que vous n’avez pas encore associé le compte de service à d’autres projets. Accédez à la Google Cloud Console, ouvrez le service IAM et sélectionnez le projet. Ajoutez le compte de service avec le rôle que vous utilisez pour la Console à ce projet. Vous devrez répéter cette étape pour chaque projet.

Cliquez sur *Ajouter un abonnement* pour associer les informations d’identification sélectionnées à un abonnement.

Pour créer un système Cloud Volumes ONTAP à paiement à l'usage, vous devez sélectionner un projet Google Cloud associé à un abonnement à Cloud Volumes ONTAP depuis le Google Cloud Marketplace. Consultez le document de configuration de la Console https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Associer un abonnement Marketplace aux identifiants Google Cloud"^].
...
| Nom du compte de service | Si vous prévoyez d'utiliser les services link:concept-data-tiering.html["NetApp Cloud Tiering"] ou  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["Sauvegarde et récupération"^], vous devez activer le commutateur *Compte de service* puis sélectionner le compte de service qui possède le rôle prédéfini d'administrateur de stockage. 


| Étiquettes | Les étiquettes sont des métadonnées pour vos ressources Google Cloud.  La console ajoute les étiquettes au système Cloud Volumes ONTAP et aux ressources Google Cloud associées au système.  Vous pouvez ajouter jusqu'à quatre étiquettes à partir de l'interface utilisateur lors de la création d'un système, puis vous pouvez en ajouter d'autres après sa création.  Notez que l'API ne vous limite pas à quatre étiquettes lors de la création d'un système.  Pour plus d'informations sur les étiquettes, reportez-vous à https://cloud.google.com/compute/docs/labeling-resources["Documentation Google Cloud : Étiquetage des ressources"^] . 


| Nom d'utilisateur et mot de passe ONTAP | Il s’agit des informations d’identification du compte administrateur du cluster Cloud Volumes ONTAP .  Vous pouvez utiliser ces informations d'identification pour vous connecter à Cloud Volumes ONTAP via ONTAP System Manager ou l'interface de ligne de commande ONTAP .  Conservez le nom d’utilisateur par défaut _admin_ ou remplacez-le par un nom d’utilisateur personnalisé. 
|===
...
. *Topologie de déploiement* : choisissez plusieurs zones de disponibilité (AZ) (recommandé) ou une seule AZ pour la configuration de la paire haute disponibilité.
+
link:concept-ha-google-cloud.html["En savoir plus sur les modèles de déploiement HA"^] .

. *Configuration réseau et connectivité* : Sélectionnez les régions et zones concernées. Sélectionnez quatre VPC différents pour la configuration de la paire haute disponibilité, un sous-réseau dans chaque VPC, puis choisissez une règle de pare-feu.
+

NOTE: Cloud Volumes ONTAP 9.19.1 et versions ultérieures prennent en charge link:concept-ha-google-cloud.html["disques partagés pour les configurations HA à zone de disponibilité unique"], rendant l'instance de médiateur facultative. Dans cette configuration, vous n'avez pas besoin de saisir une zone de médiateur, le VPC correspondant et le sous-réseau pour la réplication des données.
...
+
Si vous choisissez l'un des packages, il vous suffit de spécifier un volume, puis de vérifier et d'approuver la configuration. Les packages préconfigurés varient selon la version de Cloud Volumes ONTAP sélectionnée et les types de configuration. Par exemple, si vous sélectionnez un déploiement sur une seule zone de disponibilité pour Cloud Volumes ONTAP 9.19.1 ou une version ultérieure, vous pouvez voir des packages préconfigurés avec des machines de la série C3 et des disques Hyperdisk Balanced. Pour les versions antérieures et les zones de disponibilité multiples, vous pouvez toujours sélectionner des machines n2-standard et des disques persistants. Vous pouvez modifier les configurations, telles que les paramètres d'IOPS et de débit, en fonction des besoins de votre charge de travail.
...
NOTE: Une vitesse d'écriture élevée et une unité de transmission maximale (MTU) de 8 896 octets sont disponibles grâce à l'option *High* write speed avec les types d'instances n2-standard-16, n2-standard-32, n2-standard-48 et n2-standard-64. De plus, la MTU plus élevée de 8 896 nécessite la sélection des VPC-1, VPC-2 et VPC-3 pour le déploiement. La vitesse d'écriture élevée et une MTU de 8 896 dépendent des fonctionnalités et ne peuvent pas être désactivées individuellement au sein d'une instance configurée. Pour plus d'informations sur VPC-1, VPC-2 et VPC-3, veuillez consulter  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-connector["Règles pour VPC-1, VPC-2 et VPC-3"^].
...
| Serveur NTP | Sélectionnez *Utiliser le domaine Active Directory* pour configurer un serveur NTP à l'aide du DNS Active Directory. Si vous devez configurer un serveur NTP avec une adresse différente, vous devez utiliser l'API. Consultez la  https://docs.netapp.com/us-en/console-automation/index.html["Documentation sur l'automatisation de la NetApp Console"^] pour plus de détails. Notez que vous pouvez configurer un serveur NTP uniquement lors de la création d'un serveur CIFS. Il n'est plus possible de le configurer après la création du serveur CIFS. 
|===
. *Profil d'utilisation, type de disque et politique de hiérarchisation* : choisissez si vous souhaitez activer les fonctionnalités d'efficacité du stockage et modifier la politique de hiérarchisation des volumes, si nécessaire.
+
Pour plus d'informations, reportez-vous àlink:task-planning-your-config-gcp.html#choose-a-volume-usage-profile["Choisissez un profil d'utilisation du volume"^] ,link:concept-data-tiering.html["Présentation de la hiérarchisation des données"^] , et https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/What_Inline_Storage_Efficiency_features_are_supported_with_CVO#["KB : Quelles fonctionnalités d’efficacité du stockage en ligne sont prises en charge avec CVO ?"^]
