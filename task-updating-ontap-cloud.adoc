...
* En suivant les notifications de mise à jour qui apparaissent sur le système
...
La version de Cloud Volumes ONTAP vers laquelle vous pouvez effectuer une mise à niveau dépend de la version que vous utilisez actuellement. Chaque version générique ou corrective d'une publication dans les tableaux suivants représente la version de base disponible pour la mise à niveau. Pour plus de détails sur les correctifs disponibles, consultez la  https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-new.html["notes de version avec gestion des versions"^] pour chaque publication.

ifdef::aws[]



==== Chemins de mise à niveau pris en charge pour AWS
...
==== Chemins de mise à niveau pris en charge pour Azure
...
==== Chemins de mise à niveau pris en charge pour Google Cloud
...
Cloud Volumes ONTAP doit être enregistré auprès du support NetApp afin de pouvoir mettre à niveau le logiciel en utilisant l'une des méthodes décrites sur cette page. Cela s'applique aussi bien au modèle pay-as-you-go (PAYGO) qu'à Bring-Your-Own License (BYOL). Vous devrez link:task-registering.html["enregistrer manuellement les systèmes PAYGO"], tandis que les systèmes BYOL sont enregistrés par défaut.
...
. Sur la tuile d’agrégat requise, cliquez sur l’icône image:icon-action.png["Une icône représentant trois points horizontaux pour des actions supplémentaires"], puis sélectionnez *Afficher les détails de l’agrégat*.
+
...
. Sur le système, cliquez sur l’image:icon-action.png["Une icône représentant trois points horizontaux pour des actions supplémentaires"]icône, puis cliquez sur *Mettre à jour Cloud Volumes ONTAP*.
