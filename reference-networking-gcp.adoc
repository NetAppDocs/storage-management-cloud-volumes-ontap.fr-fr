...
keywords: networking, network, requirements, connection, connections, subnet, vpn, internet, proxy, HTTP, private, proxy server, routing, firewall, autosupport, connection, jump host, outbound, ports, bucket, private google access, set up data requirement, gcp networking, google networking, shared vpc, host project, service project; private mode or in a project protected by VPC Service Controls 
summary: 'Configurez votre réseau Google Cloud afin que les systèmes Cloud Volumes ONTAP puissent fonctionner correctement. Vous découvrirez les exigences réseau, les adresses IP privées, les VPC, les sous-réseaux, la mise en miroir des paquets, les règles de pare-feu et les configurations de haute disponibilité pour CVO, ainsi que les exigences relatives à l"agent Console.' 
---
...
https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Consultez les autorisations VPC partagées requises décrites dans le déploiement de l'agent de console."^]
...
L'agent de la Console contacte également plusieurs points de terminaison pour les opérations quotidiennes. Pour obtenir des informations sur les points de terminaison, consultez  https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking["Afficher les points de terminaison contactés depuis l'agent de la console"^] et  https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html["Préparer le réseau pour l'utilisation de la console"^].
...
TIP: Vous recherchez des informations sur l'agent Console ? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-gcp.html["Afficher les règles de pare-feu pour l'agent de console"^]
...
[[infrastructure-manager-configuration-in-private-mode-or-protected-environments]]
=== Configuration d'Infrastructure Manager en mode privé ou dans des environnements protégés

Si vous souhaitez déployer Cloud Volumes ONTAP 9.16.1 ou une version ultérieure en mode privé ou dans un projet protégé par VPC Service Controls, vous devez effectuer quelques modifications de configuration afin que Cloud Volumes ONTAP puisse utiliser Google Cloud Infrastructure Manager comme service de déploiement au lieu de Deployment Manager, que Google finira par abandonner.

.Avant de commencer
* Assurez-vous que votre système Cloud Volumes ONTAP est en version 9.16.1 ou ultérieure. Si ce n'est pas le cas, mettez votre système à niveau. Consultez  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html["Mettre à niveau Cloud Volumes ONTAP"] pour obtenir des instructions.
* Assurez-vous que les API Google Cloud sont activées. Voir https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html#step-5-enable-google-cloud-apis["Activer les API Google Cloud"^].
* Assurez-vous que l'API Cloud Build est activée. Consultez https://console.cloud.google.com/flows/enableapi?apiid=cloudbuild.googleapis.com&redirect=https://console.cloud.google.com/build/docs/private-pools/set-up-private-pool-environment["Activez l'API Cloud Build ici"^].
* Vérifiez que le compte de service de l'agent Console dispose de toutes les autorisations standard. Assurez-vous également que le compte de service dispose des autorisations  `cloudbuild.workerpools.get` et  `cloudbuild.workerpools.list`. Reportez-vous à  https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#service-account-permissions["Autorisations Google Cloud pour l'agent de la console"^].


.Étapes
. Créez un pool de workers privé avec cette configuration dans la même région que les déploiements Cloud Volumes ONTAP. Pour obtenir des informations sur la création d’un pool de workers privé, consultez  https://docs.cloud.google.com/build/docs/private-pools/create-manage-private-pools#creating_a_new_private_pool["Google Cloud documentation : Créer et gérer des pools privés"^] et  https://cloud.google.com/build/pricing["Tarification Google Cloud Build"^].
+
Si vous déployez Cloud Volumes ONTAP dans un projet protégé par VPC Service Controls, créez le pool de workers privé dans le même projet que l'agent Console. Le pool de workers doit être en cours d'exécution. Le pool de workers public par défaut de Google Cloud Cloud Build ne peut pas s'exécuter à l'intérieur d'un périmètre de service.

+
Le pool de travailleurs doit avoir la configuration suivante :

+
** Type de machine : e2-medium
** Taille du disque : 100 Go
** Attribuer une adresse IP externe : False
** Réseau : Par défaut ou privé.
** Le sous-réseau configuré pour accéder https://cloud.google.com/apis["API Google Cloud"^]. Procédez comme suit pour garantir que le sous-réseau peut accéder aux API Google :
+
... Assurez-vous que « Private Google Access » est activé pour le sous-réseau.
... Accédez à *Niveau VPC Network > Onglet Private Service Access > Allocated IP ranges for services*.
... Sélectionnez *Allocate IP range* et allouez la plage d'adresses IP internes pour la connexion privée au Google Compute Service.
... Dans *Connexion privée aux services*, sélectionnez *Créer une connexion*.
... Sélectionnez *Connected service producer = Google Cloud Platform*.
... Attribuez l'allocation à la plage d'adresses IP de connexion privée que vous avez créée à l'étape précédente.




. Déployez ce pool de nœuds de calcul et maintenez-le actif pour la gestion de Cloud Volumes ONTAP. Google Cloud utilise ce pool pour exécuter toutes les opérations Terraform dans un environnement isolé.
. Lors du déploiement de Cloud Volumes ONTAP en mode privé, sélectionnez le nom de ce worker pool dans le champ *GCP Worker Pool*. Consultez link:task-deploying-gcp.html["Lancer Cloud Volumes ONTAP dans Google Cloud"] pour obtenir des instructions.
+
Si vous déployez Cloud Volumes ONTAP dans un projet protégé par VPC Service Controls, créez le système Cloud Volumes ONTAP à l'aide de l'API et incluez le  `workerPool` paramètre dans le corps de la requête de création.

+

NOTE: L'inclusion d'un pool de workers n'est actuellement pas prise en charge lors du déploiement Cloud Volumes ONTAP via l'interface utilisateur de la console. Pour les projets protégés par VPC Service Controls, l'API est la seule méthode prise en charge pour déployer Cloud Volumes ONTAP.
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Afficher les exigences réseau pour l'agent de console"^]
...
Pour plus d'informations sur la configuration des serveurs proxy pour l'agent Console, reportez-vous à la https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["Configurer un agent de console pour utiliser un serveur proxy"^].
